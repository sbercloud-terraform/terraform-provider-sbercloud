...
layout: "sbercloud"
page_title: "SberCloud: sbercloud_dms_kafka_instance"
description: ""
---
...
resource "sbercloud_dms_kafka_instance" "test" {
  ...
  flavor_id          = data.sbercloud_dms_kafka_flavors.test.flavor_id
  storage_spec_code  = data.sbercloud_dms_kafka_flavors.test.flavors[0].ios[0].storage_spec_code
  availability_zones = var.availability_zones
  engine_version     = "2.7"
  storage_space      = 600
  broker_num         = 3
...
* `flavor_id` - (Optional, String) Specifies the Kafka [flavor ID](https://support.sbercloud.com/intl/en-us/productdesc-kafka/Kafka-specification.html),
...
* `availability_zones` - (Required, List, ForceNew) The names of the AZ where the Kafka instances reside.
  ...
  [Learn more](https://support.sbercloud.com/intl/en-us/kafka_faq/kafka-faq-200426002.html)

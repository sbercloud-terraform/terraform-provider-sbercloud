...
layout: "sbercloud"
page_title: "sberCloud: sbercloud_dns_endpoint"
description: |-
  Manages a DNS endpoint resource within sberCloud.
---

# sbercloud_dns_endpoint

Manages a DNS endpoint resource within sberCloud.

-> For the same subnet, only one of the `sbercloud_dns_endpoint` and `sbercloud_dns_endpoint_assignment` resources
   is allowed to manage an endpoint. We recommend you use the `sbercloud_dns_endpoint_assignment` resource.
...
```bash
$ terraform import sbercloud_dns_endpoint.test <id>
```

...
layout: "sbercloud"
page_title: "sberCloud: sbercloud_dns_endpoint_assignment"
description: |-
  Manages a DNS endpoint assignment resource within sberCloud.
---

# sbercloud_dns_endpoint_assignment

Manages a DNS endpoint assignment resource within sberCloud.

-> For the same subnet, only one of the `sbercloud_dns_endpoint` and `sbercloud_dns_endpoint_assignment` resources
   is allowed to manage an endpoint. We recommend using this resource to replace the `sbercloud_dns_endpoint` resource.
...
```bash
$ terraform import sbercloud_dns_endpoint_assignment.test <id>
```

For `region` required endpoint, the resource can be imported using `<region>` and `<id>`, separated by a slash, e.g.

```bash
$ terraform import sbercloud_dns_endpoint_assignment.test <region>/<id>
```

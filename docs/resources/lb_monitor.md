...
```hcl
resource "sbercloud_lb_monitor" "monitor_tcp" {
  pool_id     = var.pool_id
  type        = "TCP"
  delay       = 5
  timeout     = 3
...
```hcl
resource "sbercloud_lb_monitor" "monitor_udp" {
  pool_id     = var.pool_id
  type        = "UDP_CONNECT"
  delay       = 5
  timeout     = 3
...
```hcl
resource "sbercloud_lb_monitor" "monitor_http" {
  pool_id        = var.pool_id
  type           = "HTTP"
  delay          = 5
  timeout        = 3
...
```
$ terraform import sbercloud_lb_monitor.monitor_1 5c20fdad-7288-11eb-b817-0255ac10158b
```

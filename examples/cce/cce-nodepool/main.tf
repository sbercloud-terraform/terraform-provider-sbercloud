...
# Create CCE Node pool with security group
resource "sbercloud_networking_secgroup" "secgroup_01" {
  name        = "secgroup_01"
...
resource "sbercloud_cce_node_pool" "node_pool_01" {
  ...
  type                     = "vm"
  os                       = "CentOS 7.6"
  security_groups          = ["${sbercloud_networking_secgroup.secgroup_01.id}"]

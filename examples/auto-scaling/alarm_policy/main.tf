...
resource "sbercloud_ces_alarmrule" "scaling_up_rule" {
  ...
  metric {
    ...
    dimensions {
      ...
      value = sbercloud_as_group.my_as_group.id
    }
  }
  condition {
    period              = 300
...
resource "sbercloud_ces_alarmrule" "scaling_down_rule" {
...
resource "sbercloud_as_policy" "scaling_up_policy" {
  scaling_policy_name = "scaling_up_policy"
  scaling_policy_type = "ALARM"
  scaling_group_id    = sbercloud_as_group.my_as_group.id
  alarm_id            = sbercloud_ces_alarmrule.scaling_up_rule.id
  cool_down_time      = 300
...
resource "sbercloud_as_policy" "scaling_down_policy" {
  scaling_policy_name = "scaling_down_policy"
  scaling_policy_type = "ALARM"
  scaling_group_id    = sbercloud_as_group.my_as_group.id
  alarm_id            = sbercloud_ces_alarmrule.scaling_down_rule.id
  cool_down_time      = 300

		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkingV2SecGroupExists(
						"sbercloud_networking_secgroup_v2.secgroup_1", &secgroup_1),
					testAccCheckNetworkingV2SecGroupExists(
						"sbercloud_networking_secgroup_v2.secgroup_2", &secgroup_2),
					testAccCheckNetworkingV2SecGroupRuleExists(
						"sbercloud_networking_secgroup_rule_v2.secgroup_rule_1", &secgroup_rule_1),
					testAccCheckNetworkingV2SecGroupRuleExists(
						"sbercloud_networking_secgroup_rule_v2.secgroup_rule_2", &secgroup_rule_2),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(
						"sbercloud_networking_secgroup_rule_v2.secgroup_rule_1", "remote_ip_prefix", "2001:558:fc00::/39"),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2SecGroupRuleExists(
						"sbercloud_networking_secgroup_rule_v2.secgroup_rule_1", &secgroup_rule_1),
					resource.TestCheckResourceAttr(
						"sbercloud_networking_secgroup_rule_v2.secgroup_rule_1", "protocol", "115"),
				),
			},
		},
	})
...
func testAccCheckNetworkingV2SecGroupRuleDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "sbercloud_networking_secgroup_rule_v2" {
			continue
		}
...
resource "sbercloud_networking_secgroup_v2" "secgroup_1" {
  name = "terraform_test_secgroup_1"
  description = "terraform security group rule acceptance test"
}

resource "sbercloud_networking_secgroup_v2" "secgroup_2" {
  name = "terraform_test_secgroup_2"
  description = "terraform security group rule acceptance test"
}
...
resource "sbercloud_networking_secgroup_rule_v2" "secgroup_rule_2" {
  direction = "ingress"
  ethertype = "IPv4"
  port_range_max = 80
  port_range_min = 80
  protocol = "tcp"
  remote_group_id = "${sbercloud_networking_secgroup_v2.secgroup_1.id}"
  security_group_id = "${sbercloud_networking_secgroup_v2.secgroup_2.id}"
}
`

...
func resourceDNSZoneV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if zone_type == "private" {
		if len(router) < 1 {
			return fmt.Errorf("The argument (router) is required when creating SberCloud DNS private zone")
		}
	}
	...
	if err != nil {
		return fmt.Errorf("Error creating SberCloud DNS zone: %s", err)
	}
	...
	log.Printf("[DEBUG] Created SberCloud DNS Zone %s: %#v", n.ID, n)
...
func resourceDNSZoneV2Read(d *schema.ResourceData, meta interface{}) error {
	...
	if err = d.Set("masters", n.Masters); err != nil {
		return fmt.Errorf("[DEBUG] Error saving masters to state for SberCloud DNS zone (%s): %s", d.Id(), err)
	}
...
func resourceDNSZoneV2Update(d *schema.ResourceData, meta interface{}) error {
	...
	if zone_type == "private" {
		if len(router) < 1 {
			return fmt.Errorf("The argument (router) is required when updating SberCloud DNS private zone")
		}
	}
	...
	if err != nil {
		return fmt.Errorf("Error updating SberCloud DNS Zone: %s", err)
	}
	...
	if d.HasChange("router") {
		...
		if zone_type == "private" {
			...
			if err != nil {
				return fmt.Errorf("Error getting SberCloud DNS Zone Router: %s", err)
			}
...
func resourceDNSZoneV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error deleting SberCloud DNS Zone: %s", err)
	}
...
func waitForDNSZone(dnsClient *golangsdk.ServiceClient, zoneId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		log.Printf("[DEBUG] SberCloud DNS Zone (%s) current status: %s", zone.ID, zone.Status)
	...
	return func() (interface{}, string, error) {
		...
		for i := range zone.Routers {
			if routerId == zone.Routers[i].RouterID {
				log.Printf("[DEBUG] SberCloud DNS Zone (%s) Router (%s) current status: %s",

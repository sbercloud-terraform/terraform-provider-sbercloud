...
func TestAccDataSourceDcHostedConnects_basic(t *testing.T) {
	...
	dataSource := "data.sbercloud_dc_hosted_connects.test"
...
func testDataSourceDcHostedConnects_base(name string) string {
	return fmt.Sprintf(`
resource "sbercloud_dc_hosted_connect" "test" {
  name               = "%s"
...
data "sbercloud_dc_hosted_connects" "test" {
  depends_on = [sbercloud_dc_hosted_connect.test]
}

locals {
  hosting_id = data.sbercloud_dc_hosted_connects.test.hosted_connects.0.hosting_id
}
data "sbercloud_dc_hosted_connects" "hosting_id_filter" {
  hosting_id = [data.sbercloud_dc_hosted_connects.test.hosted_connects.0.hosting_id]
}

output "hosting_id_filter_is_useful" {
  value = length(data.sbercloud_dc_hosted_connects.hosting_id_filter.hosted_connects) > 0 && alltrue(
    [for v in data.sbercloud_dc_hosted_connects.hosting_id_filter.hosted_connects[*].hosting_id : 
      v == local.hosting_id]
  )
}

locals {
  hosted_connect_id = data.sbercloud_dc_hosted_connects.test.hosted_connects.0.id
}
data "sbercloud_dc_hosted_connects" "hosted_connect_id_filter" {
  hosted_connect_id = [data.sbercloud_dc_hosted_connects.test.hosted_connects.0.id]
}
output "hosted_connect_id_filter_is_useful" {
  value = length(data.sbercloud_dc_hosted_connects.hosted_connect_id_filter.hosted_connects) > 0 && alltrue(
    [for v in data.sbercloud_dc_hosted_connects.hosted_connect_id_filter.hosted_connects[*].id : 
      v == local.hosted_connect_id]
  )
}

locals {
  name = data.sbercloud_dc_hosted_connects.test.hosted_connects.0.name
}
data "sbercloud_dc_hosted_connects" "name_filter" {
  name = [data.sbercloud_dc_hosted_connects.test.hosted_connects.0.name]
}
output "name_filter_is_useful" {
  value = length(data.sbercloud_dc_hosted_connects.name_filter.hosted_connects) > 0 && alltrue(
    [for v in data.sbercloud_dc_hosted_connects.name_filter.hosted_connects[*].name : 
      v == local.name]
  )
}

data "sbercloud_dc_hosted_connects" "sort_filter" {
  depends_on = [sbercloud_dc_hosted_connect.test]
...
output "sort_filter_is_useful" {
  value = length(data.sbercloud_dc_hosted_connects.sort_filter.hosted_connects) > 0
}

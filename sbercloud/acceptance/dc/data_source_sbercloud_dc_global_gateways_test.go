...
func TestAccDataSourceDcGlobalGateways_basic(t *testing.T) {
	var (
		...
		dataSource = "data.sbercloud_dc_global_gateways.test"
		dc         = acceptance.InitDataSourceCheck(dataSource)

		bySortAsc   = "data.sbercloud_dc_global_gateways.filter_by_sort_asc"
		dcBySortAsc = acceptance.InitDataSourceCheck(bySortAsc)

		bySortDesc   = "data.sbercloud_dc_global_gateways.filter_by_sort_desc"
		dcBySortDesc = acceptance.InitDataSourceCheck(bySortDesc)

		byFields   = "data.sbercloud_dc_global_gateways.filter_by_fields"
		dcByFields = acceptance.InitDataSourceCheck(byFields)

		byGlobalGatewayIDs   = "data.sbercloud_dc_global_gateways.filter_by_global_gateway_ids"
		dcByGlobalGatewayIDs = acceptance.InitDataSourceCheck(byGlobalGatewayIDs)

		byNames   = "data.sbercloud_dc_global_gateways.filter_by_names"
		dcByNames = acceptance.InitDataSourceCheck(byNames)

		byEnterpriseProjectIDs   = "data.sbercloud_dc_global_gateways.filter_by_enterprise_project_ids"
		dcByEnterpriseProjectIDs = acceptance.InitDataSourceCheck(byEnterpriseProjectIDs)

		byStatuses   = "data.sbercloud_dc_global_gateways.filter_by_statuses"
...
func testDataSourceDcGlobalGateways_base(name string) string {
	return fmt.Sprintf(`
resource "sbercloud_dc_global_gateway" "test1" {
  name           = "%[1]s_1"
...
resource "sbercloud_dc_global_gateway" "test2" {
  name           = "%[1]s_2"
  description    = "test description"
  bgp_asn        = 10
  address_family = "ipv4"
...
data "sbercloud_dc_global_gateways" "test" {
  depends_on = [
    sbercloud_dc_global_gateway.test1,
    sbercloud_dc_global_gateway.test2,
  ]
}

# filter by sort
data "sbercloud_dc_global_gateways" "filter_by_sort_asc" {
  depends_on = [
    sbercloud_dc_global_gateway.test1,
    sbercloud_dc_global_gateway.test2,
  ]
...
data "sbercloud_dc_global_gateways" "filter_by_sort_desc" {
  depends_on = [
    sbercloud_dc_global_gateway.test1,
    sbercloud_dc_global_gateway.test2,
  ]
...
locals {
  gateways_length = length(data.sbercloud_dc_global_gateways.filter_by_sort_asc.gateways)
  asc_first_id = data.sbercloud_dc_global_gateways.filter_by_sort_asc.gateways[0].id
  desc_last_id = data.sbercloud_dc_global_gateways.filter_by_sort_desc.gateways[local.gateways_length - 1].id
}
...
# filter by fields
data "sbercloud_dc_global_gateways" "filter_by_fields" {
  depends_on = [
    sbercloud_dc_global_gateway.test1,
  ]
...
output "filter_by_fields_is_useful" {
  value = length(data.sbercloud_dc_global_gateways.filter_by_fields.gateways) > 0
}

# filter by global_gateway_ids
locals {
  id = data.sbercloud_dc_global_gateways.test.gateways[0].id
}

data "sbercloud_dc_global_gateways" "filter_by_global_gateway_ids" {
  global_gateway_ids = [local.id]
}

locals {
  filter_by_global_gateway_ids_result = [
    for v in data.sbercloud_dc_global_gateways.filter_by_global_gateway_ids.gateways[*].id : v == local.id
  ]
}
...
# filter by names
locals {
  name = data.sbercloud_dc_global_gateways.test.gateways[0].name
}

data "sbercloud_dc_global_gateways" "filter_by_names" {
  names = [local.name]
}

locals {
  filter_by_names_result = [
    for v in data.sbercloud_dc_global_gateways.filter_by_names.gateways[*].name : v == local.name
  ]
}
...
locals {
  enterprise_project_id = data.sbercloud_dc_global_gateways.test.gateways[0].enterprise_project_id
}

data "sbercloud_dc_global_gateways" "filter_by_enterprise_project_ids" {
  enterprise_project_ids = [local.enterprise_project_id]
}

locals {
  filter_by_enterprise_project_ids_result = [
    for v in data.sbercloud_dc_global_gateways.filter_by_enterprise_project_ids.gateways[*].enterprise_project_id :
...
# filter by statuses
locals {
  status = data.sbercloud_dc_global_gateways.test.gateways[0].status
}

data "sbercloud_dc_global_gateways" "filter_by_statuses" {
  statuses = [local.status]
}

locals {
  filter_by_statuses_result = [
    for v in data.sbercloud_dc_global_gateways.filter_by_statuses.gateways[*].status : v == local.status
  ]
}

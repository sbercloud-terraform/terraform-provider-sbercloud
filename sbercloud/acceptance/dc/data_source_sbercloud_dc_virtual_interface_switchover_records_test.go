...
func TestAccDataSourceDcVirtualInterfaceSwitchoverRecords_basic(t *testing.T) {
	dataSource := "data.sbercloud_dc_virtual_interface_switchover_records.test"
...
data "sbercloud_dc_virtual_interface_switchover_records" "test" {
  depends_on = [sbercloud_dc_virtual_interface_switchover.test]
}

locals {
  resource_id    = sbercloud_dc_virtual_interface.test.id
  filter_records = data.sbercloud_dc_virtual_interface_switchover_records.resource_id_filter.switchover_test_records
}
data "sbercloud_dc_virtual_interface_switchover_records" "resource_id_filter" {
  depends_on = [sbercloud_dc_virtual_interface_switchover.test]

...
func TestAccDatasourceDcGlobalGatewayPeerLinks_basic(t *testing.T) {
	var (
		dataSource = "data.sbercloud_dc_global_gateway_peer_links.test"
		dc         = acceptance.InitDataSourceCheck(dataSource)

		bySort   = "data.sbercloud_dc_global_gateway_peer_links.filter_by_sort"
		dcBySort = acceptance.InitDataSourceCheck(bySort)

		byFields   = "data.sbercloud_dc_global_gateway_peer_links.filter_by_fields"
		dcByFields = acceptance.InitDataSourceCheck(byFields)

		byPeerLinkIDs   = "data.sbercloud_dc_global_gateway_peer_links.filter_by_peer_link_ids"
		dcByPeerLinkIDs = acceptance.InitDataSourceCheck(byPeerLinkIDs)

		byNames   = "data.sbercloud_dc_global_gateway_peer_links.filter_by_names"
...
func testAccDatasourceDcGlobalGatewayPeerLinks_basic() string {
	return fmt.Sprintf(`
data "sbercloud_dc_global_gateway_peer_links" "test" {
  global_dc_gateway_id = "%[1]s"
}

# filter by sort (Just determine whether the data exists)
data "sbercloud_dc_global_gateway_peer_links" "filter_by_sort" {
  global_dc_gateway_id = "%[1]s"
  sort_key             = "id"
  sort_dir             = "desc"
}
...
output "filter_by_sort_is_useful" {
  value = length(data.sbercloud_dc_global_gateway_peer_links.filter_by_sort.peer_links) > 0
}

# filter by fields (Just determine whether the data exists)
data "sbercloud_dc_global_gateway_peer_links" "filter_by_fields" {
  global_dc_gateway_id = "%[1]s"
  fields               = ["name", "id"]
}

output "filter_by_fields_is_useful" {
  value = length(data.sbercloud_dc_global_gateway_peer_links.filter_by_fields.peer_links) > 0
}

# filter by peer_link_ids
locals {
  id = data.sbercloud_dc_global_gateway_peer_links.test.peer_links[0].id
}

data "sbercloud_dc_global_gateway_peer_links" "filter_by_peer_link_ids" {
  global_dc_gateway_id = "%[1]s"
...
locals {
  filter_by_peer_link_ids_result = [
    for v in data.sbercloud_dc_global_gateway_peer_links.filter_by_peer_link_ids.peer_links[*].id : v == local.id
  ]
}
...
# filter by names
locals {
  name = data.sbercloud_dc_global_gateway_peer_links.test.peer_links[0].name
}

data "sbercloud_dc_global_gateway_peer_links" "filter_by_names" {
  global_dc_gateway_id = "%[1]s"
  names                = [local.name]
}
...
locals {
  filter_by_names_result = [
    for v in data.sbercloud_dc_global_gateway_peer_links.filter_by_names.peer_links[*].name : v == local.name
  ]
}

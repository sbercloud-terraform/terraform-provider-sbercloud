...
func TestAccResourceDcGlobalGatewayRouteTable_basic(t *testing.T) {
	...
	rName := "sbercloud_dc_global_gateway_route_table.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "gdgw_id",
						"sbercloud_dc_global_gateway.test", "id"),
					...
					resource.TestCheckResourceAttrPair(rName, "nexthop",
						"sbercloud_dc_virtual_interface.test", "vif_peers.0.id"),
...
resource "sbercloud_dc_global_gateway" "test" {
  name           = "%[2]s"
...
resource "sbercloud_dc_virtual_interface" "test" {
  ...
  enable_nqa        = false
  service_type      = "GDGW"
  gateway_id        = sbercloud_dc_global_gateway.test.id
...
resource "sbercloud_dc_global_gateway_route_table" "test" {
  gdgw_id     = sbercloud_dc_global_gateway.test.id
  type        = "vif_peer"
  destination = "2.2.3.0/30"
  nexthop     = sbercloud_dc_virtual_interface.test.vif_peers[0].id
  description = "test description"
}

...
func TestAccDataSourceDcConnectGatewayGeips_basic(t *testing.T) {
	dataSource := "data.sbercloud_dc_connect_gateway_geips.test"
...
data "sbercloud_dc_connect_gateway_geips" "test" {
  depends_on = [sbercloud_dc_connect_gateway_geip_associate.test]
...
data "sbercloud_dc_connect_gateway_geips" "global_eip_id_filter" {
  depends_on = [sbercloud_dc_connect_gateway_geip_associate.test]
...
output "global_eip_id_filter_is_useful" {
  value = length(data.sbercloud_dc_connect_gateway_geips.global_eip_id_filter.global_eips) > 0 && alltrue(
  [for v in data.sbercloud_dc_connect_gateway_geips.global_eip_id_filter.global_eips[*].global_eip_id : v == local.global_eip_id]
  )
}

locals {
  status = data.sbercloud_dc_connect_gateway_geips.test.global_eips[0].status
}
data "sbercloud_dc_connect_gateway_geips" "status_filter" {
  depends_on = [sbercloud_dc_connect_gateway_geip_associate.test]
...
output "status_filter_is_useful" {
  value = length(data.sbercloud_dc_connect_gateway_geips.status_filter.global_eips) > 0 && alltrue(
  [for v in data.sbercloud_dc_connect_gateway_geips.status_filter.global_eips[*].status : v == local.status]
  )
}

data "sbercloud_dc_connect_gateway_geips" "sort_filter" {
  depends_on = [sbercloud_dc_connect_gateway_geip_associate.test]

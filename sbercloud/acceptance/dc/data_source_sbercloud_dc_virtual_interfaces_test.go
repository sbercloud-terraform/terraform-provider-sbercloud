...
func TestAccDatasourceVirtualInterface_basic(t *testing.T) {
	var (
		...
		rName = "data.sbercloud_dc_virtual_interfaces.test"
...
resource "sbercloud_dc_virtual_gateway" "test" {
  vpc_id      = sbercloud_vpc.test.id
  name        = "%[1]s"
  description = "Created by acc test"

  local_ep_group = [
    sbercloud_vpc.test.cidr,
  ]
}

resource "sbercloud_dc_virtual_interface" "test" {
  direct_connect_id = "%[2]s"
  vgw_id            = sbercloud_dc_virtual_gateway.test.id
  name              = "%[1]s"
  description       = "Created by acc test"
  type              = "private"
  route_mode        = "static"
...
data "sbercloud_dc_virtual_interfaces" "test" {
  depends_on = [sbercloud_dc_virtual_interface.test]
}

data "sbercloud_dc_virtual_interfaces" "virtual_interface_id_filter" {
  virtual_interface_id = sbercloud_dc_virtual_interface.test.id
}
...
output "virtual_interface_id_filter_is_useful" {
  value = length(data.sbercloud_dc_virtual_interfaces.virtual_interface_id_filter.virtual_interfaces) > 0 && alltrue(
    [for v in data.sbercloud_dc_virtual_interfaces.virtual_interface_id_filter.virtual_interfaces[*].id : v == 
  local.virtual_interface_id]
  )  
}

data "sbercloud_dc_virtual_interfaces" "name_filter" {
  name = sbercloud_dc_virtual_interface.test.name
}
...
output "name_filter_is_useful" {
  value = length(data.sbercloud_dc_virtual_interfaces.name_filter.virtual_interfaces) > 0 && alltrue(
    [for v in data.sbercloud_dc_virtual_interfaces.name_filter.virtual_interfaces[*].name : v == local.name]
  )
}

data "sbercloud_dc_virtual_interfaces" "status_filter" {
  status = sbercloud_dc_virtual_interface.test.status
}
...
output "status_filter_is_useful" {
  value = length(data.sbercloud_dc_virtual_interfaces.status_filter.virtual_interfaces) > 0 && alltrue(
    [for v in data.sbercloud_dc_virtual_interfaces.status_filter.virtual_interfaces[*].status : v == local.status]
  )
}

data "sbercloud_dc_virtual_interfaces" "direct_connect_id_filter" {
	direct_connect_id = sbercloud_dc_virtual_interface.test.direct_connect_id
  }
  ...
  output "direct_connect_id_filter_is_useful" {
	value = length(data.sbercloud_dc_virtual_interfaces.direct_connect_id_filter.virtual_interfaces) > 0 && alltrue(
	  [for v in data.sbercloud_dc_virtual_interfaces.direct_connect_id_filter.virtual_interfaces[*].
	direct_connect_id : v == local.direct_connect_id]
	)
  }

data "sbercloud_dc_virtual_interfaces" "vgw_id_filter" {
  vgw_id = sbercloud_dc_virtual_interface.test.vgw_id
}
...
output "vgw_id_filter_is_useful" {
  value = length(data.sbercloud_dc_virtual_interfaces.vgw_id_filter.virtual_interfaces) > 0 && alltrue(
    [for v in data.sbercloud_dc_virtual_interfaces.vgw_id_filter.virtual_interfaces[*].vgw_id : v == local.vgw_id]
  )
}

data "sbercloud_dc_virtual_interfaces" "enterprise_project_id_filter" {
  enterprise_project_id = sbercloud_dc_virtual_interface.test.enterprise_project_id
}
...
output "enterprise_project_id_filter_is_useful" {
  value = length(data.sbercloud_dc_virtual_interfaces.enterprise_project_id_filter.virtual_interfaces) > 0 && alltrue(
    [for v in data.sbercloud_dc_virtual_interfaces.enterprise_project_id_filter.virtual_interfaces[*].

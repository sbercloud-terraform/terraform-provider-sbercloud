...
func TestAccDataSourceDcConnectGateways_basic(t *testing.T) {
	dataSource := "data.sbercloud_dc_connect_gateways.test"
...
data "sbercloud_dc_connect_gateways" "test" {
  depends_on = [sbercloud_dc_connect_gateway.test]
}

locals {
  id = sbercloud_dc_connect_gateway.test.id
}
data "sbercloud_dc_connect_gateways" "id_filter" {
  connect_gateway_id = [sbercloud_dc_connect_gateway.test.id]
}
output "id_filter_is_useful" {
  value = length(data.sbercloud_dc_connect_gateways.id_filter.connect_gateways) > 0 && alltrue(
  [for v in data.sbercloud_dc_connect_gateways.id_filter.connect_gateways[*].id : v == local.id]
  )
}

data "sbercloud_dc_connect_gateways" "name_filter" {
  depends_on = [sbercloud_dc_connect_gateway.test]

  name = ["%[2]s"]
}
output "name_filter_is_useful" {
  value = length(data.sbercloud_dc_connect_gateways.name_filter.connect_gateways) > 0 && alltrue(
  [for v in data.sbercloud_dc_connect_gateways.name_filter.connect_gateways[*].name : v == "%[2]s"]
  )
}

data "sbercloud_dc_connect_gateways" "sort_filter" {
  depends_on = [sbercloud_dc_connect_gateway.test]
...
output "sort_filter_is_useful" {
  value = length(data.sbercloud_dc_connect_gateways.sort_filter.connect_gateways) > 0
}

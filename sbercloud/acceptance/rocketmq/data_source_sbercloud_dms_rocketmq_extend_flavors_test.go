...
func TestAccDataSourceDmsRocketmqExtendFlavors_basic(t *testing.T) {
	dataSource := "data.sbercloud_dms_rocketmq_extend_flavors.test"
...
data "sbercloud_dms_rocketmq_extend_flavors" "all" {
  depends_on = [sbercloud_dms_rocketmq_instance.test]
...
locals {
  test_refer   = data.sbercloud_dms_rocketmq_extend_flavors.all.flavors[0]
  test_results = data.sbercloud_dms_rocketmq_extend_flavors.test
}

...
func TestAccDmsRocketMQInstance_basic(t *testing.T) {
	...
	resourceName := "sbercloud_dms_rocketmq_instance.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "engine_version", "data.sbercloud_dms_rocketmq_flavors.test", "versions.0"),
					resource.TestCheckResourceAttrPair(resourceName, "flavor_id", "data.sbercloud_dms_rocketmq_flavors.test", "flavors.0.id"),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "engine_version", "data.sbercloud_dms_rocketmq_flavors.test", "versions.0"),
					resource.TestCheckResourceAttrPair(resourceName, "flavor_id", "data.sbercloud_dms_rocketmq_flavors.test", "flavors.1.id"),
...
locals {
  query_results = data.sbercloud_dms_rocketmq_flavors.test
...
locals {
  query_results = data.sbercloud_dms_rocketmq_flavors.test
  flavor        = data.sbercloud_dms_rocketmq_flavors.test.flavors[0]
  newFlavor     = data.sbercloud_dms_rocketmq_flavors.test.flavors[1]
}
...
resource "sbercloud_vpc_eip" "test_eip" {
  publicip {
    type = "5_bgp"
  }
...
locals {
  query_results = data.sbercloud_dms_rocketmq_flavors.test
  flavor        = data.sbercloud_dms_rocketmq_flavors.test.flavors[0]
  publicip_id   = "${sbercloud_vpc_eip.test_eip[0].id},${sbercloud_vpc_eip.test_eip[1].id},${sbercloud_vpc_eip.test_eip[2].id}"
}
...
locals {
  query_results = data.sbercloud_dms_rocketmq_flavors.test
  flavor        = data.sbercloud_dms_rocketmq_flavors.test.flavors[0]
  publicip_id   = format("%%s,%%s,%%s,%%s,%%s,%%s", sbercloud_vpc_eip.test_eip[0].id,sbercloud_vpc_eip.test_eip[1].id,
  sbercloud_vpc_eip.test_eip[2].id,sbercloud_vpc_eip.test_eip[3].id,sbercloud_vpc_eip.test_eip[4].id,sbercloud_vpc_eip.test_eip[5].id)
}
...
resource "sbercloud_dms_rocketmq_instance" "test" {
  ...
  storage_space     = 300
  enable_publicip   = true
  publicip_id       = sbercloud_vpc_eip.test.id
}
`, acceptance.TestBaseNetwork(name), name)
}
...
}`, acceptance.TestBaseNetwork(name), name, epsId)
}

...
func TestAccDatasourceRocketmqFlavors_basic(t *testing.T) {
	dataSourceName := "data.sbercloud_dms_rocketmq_flavors.test"
...
locals {
  flavor             = data.sbercloud_dms_rocketmq_flavors.test.flavors[0]
...
data "sbercloud_dms_rocketmq_flavors" "availability_zones_filter" {
  availability_zones = local.availability_zones
}

output "availability_zones_filter_is_useful" {
  value = alltrue([for v in data.sbercloud_dms_rocketmq_flavors.availability_zones_filter.flavors[*].ios :
...
data "sbercloud_dms_rocketmq_flavors" "arch_type_filter" {
  arch_type = local.arch_type
}

output "arch_type_filter_is_useful" {
  value = alltrue([for v in data.sbercloud_dms_rocketmq_flavors.arch_type_filter.flavors[*].arch_types : contains(v, local.arch_type)])
}

data "sbercloud_dms_rocketmq_flavors" "charging_mode_filter" {
  charging_mode = local.charging_mode
}

output "charging_modes_is_useful" {
  value = alltrue([for v in data.sbercloud_dms_rocketmq_flavors.charging_mode_filter.flavors[*].charging_modes : contains(v, local.charging_mode)])
}

data "sbercloud_dms_rocketmq_flavors" "flavor_id_filter" {
  flavor_id = local.flavor_id
}

output "flavor_id_is_useful" {
  value = alltrue([for v in data.sbercloud_dms_rocketmq_flavors.flavor_id_filter.flavors[*].id : v == local.flavor_id])
}

data "sbercloud_dms_rocketmq_flavors" "storage_spec_code_filter" {
  storage_spec_code = local.storage_spec_code
}

output "storage_spec_code_filter_is_useful" {
  value = alltrue([for v in data.sbercloud_dms_rocketmq_flavors.storage_spec_code_filter.flavors[*].ios :
  alltrue([for io in v : io.storage_spec_code == local.storage_spec_code])])
}

data "sbercloud_dms_rocketmq_flavors" "type_filter" { 
  type = local.type
}

output "type_filter_is_useful" {
  value = alltrue([for v in data.sbercloud_dms_rocketmq_flavors.type_filter.flavors[*].type : v == local.type])
}
`
}

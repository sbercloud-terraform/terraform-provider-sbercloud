...
func TestAccDataKpsKeypairs_basic(t *testing.T) {
	var (
		...
		resourceName = "data.sbercloud_kps_keypairs.test"
		dc           = acceptance.InitDataSourceCheck(resourceName)

		byName   = "data.sbercloud_kps_keypairs.filter_by_name"
		dcByName = acceptance.InitDataSourceCheck(byName)

		byPublicKey   = "data.sbercloud_kps_keypairs.filter_by_public_key"
		dcByPublicKey = acceptance.InitDataSourceCheck(byPublicKey)

		byFingerprint   = "data.sbercloud_kps_keypairs.filter_by_fingerprint"
...
data "sbercloud_kps_keypairs" "test" {
  depends_on = [sbercloud_kps_keypair.test]
}

# Filter by name
locals {
  name = data.sbercloud_kps_keypairs.test.keypairs.0.name
}

data "sbercloud_kps_keypairs" "filter_by_name" {
  depends_on = [sbercloud_kps_keypair.test]
...
locals {
  name_filter_result = [
    for v in data.sbercloud_kps_keypairs.filter_by_name.keypairs[*].name : v == local.name
  ]
}
...
locals {
  public_key = data.sbercloud_kps_keypairs.test.keypairs.0.public_key
}

data "sbercloud_kps_keypairs" "filter_by_public_key" {
  depends_on = [sbercloud_kps_keypair.test]
...
locals {
  public_key_filter_result = [
    for v in data.sbercloud_kps_keypairs.filter_by_public_key.keypairs[*].public_key : v == local.public_key
  ]
}
...
locals {
  fingerprint = data.sbercloud_kps_keypairs.test.keypairs.0.fingerprint
}

data "sbercloud_kps_keypairs" "filter_by_fingerprint" {
  depends_on = [sbercloud_kps_keypair.test]
...
locals {
  fingerprint_filter_result = [
    for v in data.sbercloud_kps_keypairs.filter_by_fingerprint.keypairs[*].fingerprint : v == local.fingerprint
  ]
}

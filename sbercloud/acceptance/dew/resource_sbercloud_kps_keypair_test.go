...
func TestAccKeypair_basic(t *testing.T) {
	...
	rName := "sbercloud_kps_keypair.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(rName, "key_file", acceptance.SBC_KPS_KEY_FILE_PATH),
					resource.TestCheckResourceAttr(rName, "user_id", acceptance.SBC_USER_ID),
					resource.TestCheckResourceAttr(rName, "scope", "user"),

					resource.TestCheckResourceAttrPair(rName, "kms_key_id", "sbercloud_kms_key.test", "id"),
					resource.TestCheckResourceAttrPair(rName, "kms_key_name", "sbercloud_kms_key.test", "key_alias"),
...
resource "sbercloud_kps_keypair" "test" {
  ...
  kms_key_name    = sbercloud_kms_key.test.key_alias
  description     = "test description"
  key_file        = "%[4]s"
}
`, testAccKeypair_base(name), name, acceptance.SBC_USER_ID, acceptance.SBC_KPS_KEY_FILE_PATH)
}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "public_key", "sbercloud_kps_keypair.test-base", "public_key"),
...
resource "sbercloud_kps_keypair" "test-base" {
  name            = "%[1]s-base"
  scope           = "user"
  encryption_type = "default"
  description     = "test description"
...
`, name, acceptance.SBC_KPS_KEY_FILE_PATH)
}
...
resource "sbercloud_kps_keypair" "test" {
  ...
  scope       = "account"
  description = "test description"
  public_key  = sbercloud_kps_keypair.test-base.public_key
}
...
`, testAccKeypair_import_base(name), name, acceptance.SBC_KPS_KEY_FILE_PATH)
}

...
func getVaultResourceFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := cfg.NewServiceClient("cbr", acceptance.SBC_REGION_NAME)
	if err != nil {
...
variable "volume_configuration" {
  ...
  default = [
    ...
    {volume_type = "ESSD", size = 100, device_type = "VBD"},
...
resource "sbercloud_cbr_vault" "test" {
  ...
  resources {
    ...
    excludes  = slice(sbercloud_compute_volume_attach.test[*].volume_id, 0, 2)
  }
...
resource "sbercloud_cbr_vault" "test" {
  ...
  resources {
    server_id = sbercloud_compute_instance.test.id
    excludes  = slice(sbercloud_compute_volume_attach.test[*].volume_id, 1, 3)
  }
...
resource "sbercloud_cbr_vault" "test" {
  ...
  resources {
    includes = slice(sbercloud_compute_volume_attach.test[*].volume_id, 0, 2)
  }
}
`, basicConfig, name)
}
...
resource "sbercloud_cbr_vault" "test" {
  ...
  resources {
    includes = slice(sbercloud_compute_volume_attach.test[*].volume_id, 1, 3)
  }
}
`, basicConfig, name)
}
...
resource "sbercloud_cbr_vault" "test" {
  ...
  resources {
    includes = slice(sbercloud_sfs_turbo.test[*].id, 0, 2)
  }
}
...
resource "sbercloud_cbr_vault" "test" {
  ...
  resources {
    includes = slice(sbercloud_sfs_turbo.test[*].id, 1, 3)
  }
}
`, testAccVault_backupTurbo_base(rName), rName)
}
...
func TestAccVault_bindPolicies(t *testing.T) {
	var (
		...
		randName     = acceptance.RandomAccResourceName()
		mainRcName   = "sbercloud_cbr_vault.test"
		legacyRcName = "sbercloud_cbr_vault.legacy"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(legacyRcName, "policy_id",
						"sbercloud_cbr_policy.backup.0", "id"),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					legacyRc.CheckResourceExists(),
					resource.TestCheckResourceAttrPair(legacyRcName, "policy_id",
						"sbercloud_cbr_policy.backup.1", "id"),
				),
			},
			{
...
resource "sbercloud_cbr_policy" "backup" {
  count = 2
...
resource "sbercloud_cbr_policy" "replication" {
  count = 2
...
`, name, acceptance.SBC_DEST_REGION, acceptance.SBC_DEST_PROJECT_ID)
}
...
resource "sbercloud_cbr_vault" "legacy" {
  name            = "%[2]s_legacy"
  type            = "server"
  protection_type = "backup"
  size            = 200
  policy_id       = sbercloud_cbr_policy.backup[0].id
}
...
resource "sbercloud_cbr_vault" "test" {
  ...
  policy {
    id = sbercloud_cbr_policy.backup[0].id
  }
  policy {
    id                   = sbercloud_cbr_policy.replication[0].id
...
resource "sbercloud_cbr_vault" "legacy" {
  ...
  protection_type = "backup"
  size            = 200
  policy_id       = sbercloud_cbr_policy.backup[1].id
}
...
resource "sbercloud_cbr_vault" "test" {
  ...
  policy {
    id = sbercloud_cbr_policy.backup[1].id
  }
  policy {
    id                   = sbercloud_cbr_policy.replication[1].id
    destination_vault_id = sbercloud_cbr_vault.destination.id
  }
}
`, testAccVault_bindPolicies_base(name), name)
...
// workspace service is not supported
...
`, basicConfig, name, acceptance.SBC_ENTERPRISE_PROJECT_ID_TEST)
}

// Locked parameter isn't supported

...
resource "sbercloud_dms_kafka_instance" "test" {
  ...
  flavor_id          = local.flavor.id
  storage_spec_code  = local.flavor.ios[0].storage_spec_code
  availability_zones = slice(data.sbercloud_availability_zones.test.names, 0, 3)

  engine_version        = "2.7"
  storage_space         = 300
  broker_num            = 3
  arch_type             = "X86"
  public_ip_ids         = sbercloud_vpc_eip.test[*].id
...
}`, acceptance.TestBaseNetwork(rName), testAccKafkaInstance_publicIpBase(3), rName, acceptance.SBC_ENTERPRISE_PROJECT_ID_TEST)
}
...
}`, acceptance.TestBaseNetwork(rName), testAccKafkaInstance_publicIpBase(4), rName, brokerNum, password, acceptance.SBC_ENTERPRISE_MIGRATE_PROJECT_ID_TEST)
}

...
func TestAccKafkaFlavorsDataSource_basic(t *testing.T) {
	dataSourceName := "data.sbercloud_dms_kafka_flavors.test"
...
data "sbercloud_dms_kafka_flavors" "baisc" {
  type = "cluster"
}
...
locals {
  test_refer   = data.sbercloud_dms_kafka_flavors.baisc.flavors[0]
  test_results = data.sbercloud_dms_kafka_flavors.test
}

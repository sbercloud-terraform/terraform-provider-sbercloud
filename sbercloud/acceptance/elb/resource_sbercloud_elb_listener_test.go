...
// API does not support ssl_early_data_enable
...
//						"sbercloud_elb_listener.quic", "id"),
...
//						"sbercloud_elb_security_policy.test", "id"),
//				),
//			},
//			{
//				ResourceName:      resourceName,
...
resource "sbercloud_elb_listener" "test" {
  ...
  protocol_port               = 8080
  loadbalancer_id             = sbercloud_elb_loadbalancer.test.id
  default_pool_id             = sbercloud_elb_pool.test.id
  advanced_forwarding_enabled = false
...
resource "sbercloud_elb_pool" "test_update" {
  name      = "%[1]s"
  protocol  = "HTTP"
  lb_method = "ROUND_ROBIN"
  type      = "instance"
...
resource "sbercloud_elb_listener" "test" {
  ...
  protocol_port               = 8080
  loadbalancer_id             = sbercloud_elb_loadbalancer.test.id
  default_pool_id             = sbercloud_elb_pool.test_update.id
  advanced_forwarding_enabled = true
...
func testAccElbV3ListenerConfig_protocol_quic(rName string) string {
	return fmt.Sprintf(`
resource "sbercloud_elb_listener" "quic" {
  name               = "%s-quic"
  protocol           = "QUIC"
  protocol_port      = 80
  loadbalancer_id    = sbercloud_elb_loadbalancer.test.id
  server_certificate = sbercloud_elb_certificate.server.id
}`, rName)
}

func testAccSecurityPolicies_tlsv13(name string) string {
	return fmt.Sprintf(`
resource "sbercloud_elb_security_policy" "test" {
  protocols = [
    "TLSv1.3"
  ]
  ciphers = [
...
resource "sbercloud_elb_listener" "test" {
  ...
  quic_listener_id        = sbercloud_elb_listener.quic.id
  enable_quic_upgrade     = "true"
  access_policy           = "white"
  ip_group                = sbercloud_elb_ipgroup.test.id
...
resource "sbercloud_elb_listener" "test" {
  ...
  security_policy_id      = sbercloud_elb_security_policy.test.id
  ...
  ca_certificate          = sbercloud_elb_certificate.client.id
  server_certificate      = sbercloud_elb_certificate.server.id
}

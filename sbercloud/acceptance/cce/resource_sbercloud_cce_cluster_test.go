		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "security_group_id",
						"sbercloud_networking_secgroup.test1", "id"),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "status", "Available"),
					resource.TestCheckResourceAttrPair(resourceName, "security_group_id",
						"sbercloud_networking_secgroup.test2", "id"),
				),
			},
		},
	})
...
`, acceptance.TestVpc(rName), rName, isAutoRenew)
}
...
resource "sbercloud_vpc_eip" "update" {
  publicip {
    type = "5_bgp"
  }
  bandwidth {
...
resource "sbercloud_cce_cluster" "test" {
  ...
  container_network_type = "overlay_l2"
  authentication_mode    = "rbac"
  eip                    = sbercloud_vpc_eip.update.address
}
`, acceptance.TestVpc(rName), rName)
}
...
`, acceptance.TestVpc(rName), rName, containerNetworkCidr)
}
...
resource "sbercloud_networking_secgroup" "test1" {
  name = "%[2]s-secgroup-1"
}
...
resource "sbercloud_cce_cluster" "test" {
  ...
  security_group_id      = sbercloud_networking_secgroup.test1.id
...
resource "sbercloud_cce_cluster" "test" {
  ...
  security_group_id      = sbercloud_networking_secgroup.test2.id

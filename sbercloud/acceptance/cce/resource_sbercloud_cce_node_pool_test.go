		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "security_groups.0",
						"sbercloud_networking_secgroup.test.0", "id"),
					resource.TestCheckResourceAttrPair(resourceName, "security_groups.1",
						"sbercloud_networking_secgroup.test.1", "id"),
				),
			},
		},
	})
...
resource "sbercloud_networking_secgroup_rule" "rule1" {
  security_group_id = sbercloud_networking_secgroup.test[0].id
  action            = "allow"
  direction         = "ingress"
  ethertype         = "IPv4"
  remote_ip_prefix  = sbercloud_vpc_subnet.eni_test[0].cidr
}

resource "sbercloud_networking_secgroup_rule" "rule2" {
  security_group_id = sbercloud_networking_secgroup.test[0].id
  action            = "allow"
  direction         = "ingress"
  ethertype         = "IPv4"
  ports             = "10250"
  protocol          = "tcp"
  remote_ip_prefix  = sbercloud_vpc_subnet.test.cidr
}

resource "sbercloud_networking_secgroup_rule" "rule3" {
  security_group_id = sbercloud_networking_secgroup.test[0].id
  action            = "allow"
  direction         = "ingress"
  ethertype         = "IPv4"
...
resource "sbercloud_networking_secgroup_rule" "rule4" {
  security_group_id = sbercloud_networking_secgroup.test[0].id
  action            = "allow"
  direction         = "ingress"
  ethertype         = "IPv4"
...
resource "sbercloud_networking_secgroup_rule" "rule5" {
  security_group_id = sbercloud_networking_secgroup.test[0].id
  action            = "allow"
  direction         = "egress"
  ethertype         = "IPv4"
...
resource "sbercloud_networking_secgroup_rule" "rule6" {
  security_group_id = sbercloud_networking_secgroup.test[0].id
  action            = "allow"
  direction         = "ingress"
  ethertype         = "IPv4"
  remote_group_id   = sbercloud_networking_secgroup.test[0].id
}

resource "sbercloud_networking_secgroup_rule" "rule7" {
  security_group_id = sbercloud_networking_secgroup.test[0].id
  action            = "allow"
  direction         = "ingress"
  ethertype         = "IPv4"
...
resource "sbercloud_cce_node_pool" "test" {
  ...
  pod_security_groups = [
    sbercloud_networking_secgroup.test[2].id,
    sbercloud_networking_secgroup.test[3].id
  ]
...
resource "sbercloud_cce_node_pool" "test" {
  ...
  storage {
    ...
    selectors {
      ...
      match_label_metadata_cmkid     = sbercloud_kms_key.test.id
      match_label_count              = "1"
    }
...
resource "sbercloud_cce_node_pool" "test" {
  ...
  extension_scale_groups {
    ...
    spec {
      ...
      az     = data.sbercloud_availability_zones.test.names[1]
      autoscaling {
...
resource "sbercloud_cce_node_pool" "test" {
  ...
  extension_scale_groups {
    ...
    spec {
      flavor = data.sbercloud_compute_flavors.test.ids[1]
      az     = data.sbercloud_availability_zones.test.names[0]
      autoscaling {
        extension_priority = 1
        enable             = true
      }

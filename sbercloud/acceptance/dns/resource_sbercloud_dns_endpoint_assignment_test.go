...
import (
	...
	"github.com/sbercloud-terraform/terraform-provider-sbercloud/sbercloud/services/dns"
)
...
func TestAccEndpointAssignment_basic(t *testing.T) {
	var (
		endpoint   interface{}
		rName      = "sbercloud_dns_endpoint_assignment.test"
...
resource "sbercloud_vpc_subnet" "test" {
  ...
  cidr       = cidrsubnet(sbercloud_vpc.test.cidr, 4, count.index)
  gateway_ip = cidrhost(cidrsubnet(sbercloud_vpc.test.cidr, 4, count.index), 1)
}`, rName)
}
...
resource "sbercloud_dns_endpoint_assignment" "test" {
  name      = "%[2]s"
  direction = "inbound"
  ...
  dynamic "assignments" {
    for_each = range(4)
    content {
      subnet_id  = sbercloud_vpc_subnet.test[0].id
      ip_address = cidrhost(sbercloud_vpc_subnet.test[0].cidr, assignments.key + 100)
    }
  }

  assignments {
    subnet_id  = sbercloud_vpc_subnet.test[1].id
    ip_address = cidrhost(sbercloud_vpc_subnet.test[1].cidr, 100)
  }
...
resource "sbercloud_dns_endpoint_assignment" "test" {
  ...
  assignments {
    subnet_id  = sbercloud_vpc_subnet.test[1].id
    ip_address = cidrhost(sbercloud_vpc_subnet.test[1].cidr, 102)
  }
  assignments {
    subnet_id  = sbercloud_vpc_subnet.test[1].id
    ip_address = cidrhost(sbercloud_vpc_subnet.test[1].cidr, 103)
  }
...
}`, acceptance.SBC_CUSTOM_REGION_NAME, rName)
}
...
resource "sbercloud_dns_endpoint_assignment" "test" {
  ...
  assignments {
    ...
    ip_address = cidrhost(sbercloud_vpc_subnet.test.cidr, 100)
  }
  assignments {
    subnet_id  = sbercloud_vpc_subnet.test.id
    ip_address = cidrhost(sbercloud_vpc_subnet.test.cidr, 101)
  }
}`, testEndpointAssignment_regional_base(rName), acceptance.SBC_CUSTOM_REGION_NAME, rName)
}
...
resource "sbercloud_dns_endpoint_assignment" "test" {
  ...
  assignments {
    subnet_id  = sbercloud_vpc_subnet.test.id
    ip_address = cidrhost(sbercloud_vpc_subnet.test.cidr, 102)
  }
  assignments {
    subnet_id  = sbercloud_vpc_subnet.test.id
    ip_address = cidrhost(sbercloud_vpc_subnet.test.cidr, 103)
  }
}`, testEndpointAssignment_regional_base(rName), acceptance.SBC_CUSTOM_REGION_NAME, updateName)
}

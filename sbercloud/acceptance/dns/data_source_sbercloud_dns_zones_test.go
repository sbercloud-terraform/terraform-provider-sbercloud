...
func TestAccDataZones_basic(t *testing.T) {
	var (
		...
		rName = "sbercloud_dns_zone.test.0"

		all           = "data.sbercloud_dns_zones.test"
		dcForAllZones = acceptance.InitDataSourceCheck(all)

		byZoneId           = "data.sbercloud_dns_zones.filter_by_zone_id"
		dcByZoneId         = acceptance.InitDataSourceCheck(byZoneId)
		byNotFoundZoneId   = "data.sbercloud_dns_zones.filter_by_not_found_zone_id"
		dcByNotFoundZoneId = acceptance.InitDataSourceCheck(byNotFoundZoneId)

		byNameFuzzy      = "data.sbercloud_dns_zones.filter_by_name_fuzzy"
		dcByNameFuzzy    = acceptance.InitDataSourceCheck(byNameFuzzy)
		byNameExact      = "data.sbercloud_dns_zones.filter_by_name_exact"
		dcByNameExact    = acceptance.InitDataSourceCheck(byNameExact)
		byNotFoundName   = "data.sbercloud_dns_zones.filter_by_not_found_name"
		dcByNotFoundName = acceptance.InitDataSourceCheck(byNotFoundName)

		byStatus   = "data.sbercloud_dns_zones.filter_by_status"
		dcByStatus = acceptance.InitDataSourceCheck(byStatus)

		byEpsId           = "data.sbercloud_dns_zones.filter_by_eps_id"
		dcByEpsId         = acceptance.InitDataSourceCheck(byEpsId)
		byNotFoundEpsId   = "data.sbercloud_dns_zones.filter_by_not_found_eps_id"
		dcByNotFoundEpsId = acceptance.InitDataSourceCheck(byNotFoundEpsId)

		byTags           = "data.sbercloud_dns_zones.filter_by_tags"
		dcByTags         = acceptance.InitDataSourceCheck(byTags)
		byNotFoundTags   = "data.sbercloud_dns_zones.filter_by_not_found_tags"
		dcByNotFoundTags = acceptance.InitDataSourceCheck(byNotFoundTags)

		byRouterId           = "data.sbercloud_dns_zones.filter_by_router_id"
		dcByRouterId         = acceptance.InitDataSourceCheck(byRouterId)
		byNotFoundRouterId   = "data.sbercloud_dns_zones.filter_by_not_found_router_id"
		dcByNotFoundRouterId = acceptance.InitDataSourceCheck(byNotFoundRouterId)

		bySortAsc    = "data.sbercloud_dns_zones.filter_by_sort_asc"
		dcBySortAsc  = acceptance.InitDataSourceCheck(bySortAsc)
		bySortDesc   = "data.sbercloud_dns_zones.filter_by_sort_desc"
...
func testAccDataZones_base(name, randomId string) string {
	return fmt.Sprintf(`
data "sbercloud_dns_zones" "test" {
  zone_type = sbercloud_dns_zone.test.0.zone_type
}

# Filter by zone ID.
locals {
  zone_id = sbercloud_dns_zone.test.0.id
}

data "sbercloud_dns_zones" "filter_by_zone_id" {
  zone_type = sbercloud_dns_zone.test.0.zone_type
  zone_id   = local.zone_id
}

locals {
  zone_id_filter_result = [for v in data.sbercloud_dns_zones.filter_by_zone_id.zones[*].id : v == local.zone_id]
}
...
data "sbercloud_dns_zones" "filter_by_not_found_zone_id" {
  zone_type = sbercloud_dns_zone.test.0.zone_type
  zone_id   = "%[2]s"
}

output "zone_id_not_found_validation_pass" {
  value = length(data.sbercloud_dns_zones.filter_by_not_found_zone_id.zones) == 0
}

# Filter by zone name (Fuzzy search).
data "sbercloud_dns_zones" "filter_by_name_fuzzy" {
  depends_on = [sbercloud_dns_zone.test]
...
locals {
  name_fuzzy_filter_result = [for v in data.sbercloud_dns_zones.filter_by_name_fuzzy.zones[*].name :
  strcontains(v, "%[1]s")]
}
...
locals {
  zone_name = sbercloud_dns_zone.test.0.name
}

data "sbercloud_dns_zones" "filter_by_name_exact" {
  depends_on = [sbercloud_dns_zone.test]
...
locals {
  exact_name_filter_result = [for v in data.sbercloud_dns_zones.filter_by_name_exact.zones[*].name : v == local.zone_name]
}
...
data "sbercloud_dns_zones" "filter_by_not_found_name" {
  depends_on = [sbercloud_dns_zone.test]
...
output "name_not_found_validation_pass" {
  value = length(data.sbercloud_dns_zones.filter_by_not_found_name.zones) == 0
}

# By filter zone status.
data "sbercloud_dns_zones" "filter_by_status" {
  zone_type = sbercloud_dns_zone.test.0.zone_type
...
locals {
  status_filter_result = [for v in data.sbercloud_dns_zones.filter_by_status.zones[*].status : v == "ACTIVE"]
}
...
locals {
  enterprise_project_id = sbercloud_dns_zone.test.0.enterprise_project_id
}

data "sbercloud_dns_zones" "filter_by_eps_id" {
  zone_type             = sbercloud_dns_zone.test.0.zone_type
  enterprise_project_id = local.enterprise_project_id
}

locals {
  eps_id_filter_result = [for v in data.sbercloud_dns_zones.filter_by_eps_id.zones[*].enterprise_project_id :
...
data "sbercloud_dns_zones" "filter_by_not_found_eps_id" {
  zone_type             = sbercloud_dns_zone.test.0.zone_type
  enterprise_project_id = "%[2]s"
}

output "eps_id_not_found_validation_pass" {
  value = length(data.sbercloud_dns_zones.filter_by_not_found_eps_id.zones) == 0
}

# Filter by zone tags.
data "sbercloud_dns_zones" "filter_by_tags" {
  zone_type = sbercloud_dns_zone.test.0.zone_type
  tags      = join("|", [for key, value in sbercloud_dns_zone.test.0.tags : format("%%v,%%v", key, value)])
}

output "is_tags_filter_useful" {
  value = length(data.sbercloud_dns_zones.filter_by_tags.zones) >= 2
}

data "sbercloud_dns_zones" "filter_by_not_found_tags" {
  zone_type = sbercloud_dns_zone.test.0.zone_type
  tags      = "not_found_tags,not_found"
}

output "tags_not_found_validation_pass" {
  value = length(data.sbercloud_dns_zones.filter_by_not_found_tags.zones) == 0
}

# Filter in ascending order using 'name' field as key.
data "sbercloud_dns_zones" "filter_by_sort_asc" {
  zone_type = sbercloud_dns_zone.test.0.zone_type
  sort_key  = "name"
  sort_dir  = "asc"
}
...
data "sbercloud_dns_zones" "filter_by_sort_desc" {
  zone_type = sbercloud_dns_zone.test.0.zone_type
  sort_key  = "name"
  sort_dir  = "desc"
}
...
locals {
  sort_desc_filter_result = data.sbercloud_dns_zones.filter_by_sort_desc.zones
  sort_asc_first_name     = data.sbercloud_dns_zones.filter_by_sort_asc.zones[0].name
  sort_desc_last_name     = data.sbercloud_dns_zones.filter_by_sort_desc.zones[length(local.sort_desc_filter_result) - 1].name
}
...
# Filter by router Id.
locals {
  router_id = try(tolist(sbercloud_dns_zone.test.0.router)[0].router_id, "")
}

data "sbercloud_dns_zones" "filter_by_router_id" {
  zone_type = sbercloud_dns_zone.test.0.zone_type
  router_id = local.router_id
}

locals {
  router_id_filter_result = [for v in flatten(data.sbercloud_dns_zones.filter_by_router_id.zones[*].routers[*].router_id) :
  v == local.router_id]
}
...
data "sbercloud_dns_zones" "filter_by_not_found_router_id" {
  zone_type = sbercloud_dns_zone.test.0.zone_type
  router_id = "%[5]s"
}

output "router_id_not_found_validation_pass" {
  value = length(data.sbercloud_dns_zones.filter_by_not_found_router_id.zones) == 0
}
`, acceptance.RandomAccResourceName(), name, acceptance.SBC_ENTERPRISE_PROJECT_ID_TEST,
...
%[3]s
`, name, acceptance.SBC_ENTERPRISE_PROJECT_ID_TEST, testAccDataZones_base(name, randomId))
}

...
func getResolverRuleAssociatedVpc(conf *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := conf.DNSV21Client(acceptance.SBC_REGION_NAME)
	if err != nil {
...
func TestAccResolverRuleAssociate_basic(t *testing.T) {
	var (
		...
		rName         = "sbercloud_dns_resolver_rule_associate.test"
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "resolver_rule_id",
						"sbercloud_dns_resolver_rule.test", "id"),
...
resource "sbercloud_dns_endpoint" "test" {
  name      = "%[2]s"
  direction = "inbound"
...
resource "sbercloud_dns_resolver_rule" "test" {
  name        = "%[2]s"
  domain_name = "terraform.test.com."
  endpoint_id = sbercloud_dns_endpoint.test.id

  ip_addresses {
    ip = sbercloud_dns_endpoint.test.ip_addresses[0].ip
  }
}
`, sbercloud.TestVpc(rName), rName)
}

func testResolverRuleAssociate_ruleNotExist(randomId string) string {
	return fmt.Sprintf(`
resource "sbercloud_dns_resolver_rule_associate" "test" {
...
resource "sbercloud_dns_resolver_rule_associate" "test" {
  resolver_rule_id = sbercloud_dns_resolver_rule.test.id
  vpc_id           = "%s"
}

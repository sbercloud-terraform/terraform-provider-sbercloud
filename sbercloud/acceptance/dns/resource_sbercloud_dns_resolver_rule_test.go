...
func TestAccResolverRule_basic(t *testing.T) {
	var (
		...
		rName        = "sbercloud_dns_resolver_rule.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "endpoint_id",
						"sbercloud_dns_endpoint.test", "id"),
					resource.TestCheckResourceAttrPair(rName, "ip_addresses.0.ip",
						"sbercloud_dns_endpoint.test", "ip_addresses.0.ip"),
					// Check attributtes.
...
resource "sbercloud_dns_resolver_rule" "test" {
  ...
  dynamic "ip_addresses" {
    for_each = sbercloud_dns_endpoint.test.ip_addresses[*].ip
    content {

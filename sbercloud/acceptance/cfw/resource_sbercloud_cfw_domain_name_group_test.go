...
func TestAccDomainNameGroup_basic(t *testing.T) {
	...
	rName := "sbercloud_cfw_domain_name_group.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "fw_instance_id",
						"sbercloud_cfw_firewall.test", "id"),
					resource.TestCheckResourceAttrPair(rName, "object_id",
						"sbercloud_cfw_firewall.test", "protect_objects.0.object_id"),
...
resource "sbercloud_cfw_domain_name_group" "test" {
  fw_instance_id = sbercloud_cfw_firewall.test.id
  object_id      = sbercloud_cfw_firewall.test.protect_objects[0].object_id
  name           = "%s"
  type           = 0

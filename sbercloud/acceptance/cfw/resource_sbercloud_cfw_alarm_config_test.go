...
func TestAccResourceAlarmConfig_basic(t *testing.T) {
	...
	resourceName := "sbercloud_cfw_alarm_config.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "topic_urn", "sbercloud_smn_topic.t1", "topic_urn"),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "topic_urn", "sbercloud_smn_topic.t2", "topic_urn"),
				),
			},
			{
...
resource "sbercloud_cfw_alarm_config" "test" {
  ...
  frequency_time    = 10
  severity          = "LOW"
  topic_urn         = sbercloud_smn_topic.t1.topic_urn
}
`, baseConfig, acceptance.SBC_CFW_INSTANCE_ID)
}
...
resource "sbercloud_cfw_alarm_config" "test" {
  ...
  frequency_time    = 5
  severity          = "MEDIUM,LOW"
  topic_urn         = sbercloud_smn_topic.t2.topic_urn
}
`, baseConfig, acceptance.SBC_CFW_INSTANCE_ID)
}
...
func testResourceAlarmConfig_base(name string) string {
	return fmt.Sprintf(`
resource "sbercloud_smn_topic" "t1" {
  name = "%[1]s-1"
}

resource "sbercloud_smn_topic" "t2" {
  name = "%[1]s-2"
}
`, name)
}

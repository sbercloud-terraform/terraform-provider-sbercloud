...
func TestAccDataSourceCfwIpsRules_basic(t *testing.T) {
	dataSource := "data.sbercloud_cfw_ips_rules.test"
...
output "is_default_filter_useful" {
  value = length(data.sbercloud_cfw_ips_rules.test.records) >= 1
}

data "sbercloud_cfw_ips_rules" "filter_by_id" {
  object_id = data.sbercloud_cfw_firewalls.test.records[0].protect_objects[0].object_id
  ips_id    = local.ips_id
}

output "is_id_filter_useful" {
  value = length(data.sbercloud_cfw_ips_rules.filter_by_id.records) >= 1 && alltrue(
    [for r in data.sbercloud_cfw_ips_rules.filter_by_id.records[*] : r.ips_id == local.ips_id]
  )
}

data "sbercloud_cfw_ips_rules" "filter_by_name_like" {
  object_id     = data.sbercloud_cfw_firewalls.test.records[0].protect_objects[0].object_id
  ips_name_like = local.ips_name_like
}

output "is_name_like_filter_useful" {
  value = length(data.sbercloud_cfw_ips_rules.filter_by_name_like.records) >= 1 && alltrue(
    [for r in data.sbercloud_cfw_ips_rules.filter_by_name_like.records[*] : can(regex("(?i).*web.*", r.ips_name))]
  )
}

data "sbercloud_cfw_ips_rules" "filter_by_status" {
  object_id  = data.sbercloud_cfw_firewalls.test.records[0].protect_objects[0].object_id
  ips_status = local.ips_status
}

output "is_status_filter_useful" {
  value = length(data.sbercloud_cfw_ips_rules.filter_by_status.records) >= 1 && alltrue(
    [for r in data.sbercloud_cfw_ips_rules.filter_by_status.records[*] : r.ips_status == local.ips_status]
  )
}

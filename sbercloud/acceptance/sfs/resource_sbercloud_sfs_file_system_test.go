...
func TestAccSFSFileSystemV2_basic(t *testing.T) {
	...
	resourceName := "sbercloud_sfs_file_system.sfs_1"
...
func testAccCheckSFSFileSystemV2Destroy(s *terraform.State) error {
	...
	sfsClient, err := config.SfsV2Client(acceptance.SBC_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating SberCloud sfs client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "sbercloud_sfs_file_system" {
			continue
		}
...
resource "sbercloud_sfs_file_system" "sfs_1" {
  ...
  access_to    = sbercloud_vpc.test.id
  access_level = "rw"
...
`, rName, rName, acceptance.SBC_ENTERPRISE_PROJECT_ID_TEST)
}

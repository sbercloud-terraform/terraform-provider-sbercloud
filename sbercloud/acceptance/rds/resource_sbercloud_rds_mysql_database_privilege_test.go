...
func TestAccMysqlDatabasePrivilege_basic(t *testing.T) {
	...
	rName := "sbercloud_rds_mysql_database_privilege.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "db_name",
						"sbercloud_rds_mysql_database.test", "name"),
					resource.TestCheckResourceAttrPair(rName, "users.0.name",
						"sbercloud_rds_mysql_account.test_1", "name"),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "db_name",
						"sbercloud_rds_mysql_database.test", "name"),
					resource.TestCheckResourceAttrPair(rName, "users.0.name",
						"sbercloud_rds_mysql_account.test_2", "name"),
...
resource "sbercloud_rds_mysql_account" "test_1" {
...
resource "sbercloud_rds_mysql_database_privilege" "test" {
  instance_id = sbercloud_rds_instance.test.id
  db_name     = sbercloud_rds_mysql_database.test.name

  users {
    name = sbercloud_rds_mysql_account.test_1.name
  }
}
...
resource "sbercloud_rds_mysql_account" "test_2" {
  instance_id = sbercloud_rds_instance.test.id
  name        = "%s_2"
  password    = "Test@12345678"
}
...
resource "sbercloud_rds_mysql_database_privilege" "test" {
  ...
  users {
    name     = sbercloud_rds_mysql_account.test_2.name
    readonly = true
  }
}
`, testMysqlDatabase_basic(rName, rName), rName)

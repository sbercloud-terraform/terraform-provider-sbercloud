...
func TestAccDataSourceApplications_basic(t *testing.T) {
	var (
		dataSource = "data.sbercloud_apig_applications.test"
		dc         = acceptance.InitDataSourceCheck(dataSource)
		rName      = acceptance.RandomAccResourceName()

		byId   = "data.sbercloud_apig_applications.filter_by_id"
		dcById = acceptance.InitDataSourceCheck(byId)

		byName   = "data.sbercloud_apig_applications.filter_by_name"
		dcByName = acceptance.InitDataSourceCheck(byName)

		byAppKey   = "data.sbercloud_apig_applications.filter_by_app_key"
		dcByAppKey = acceptance.InitDataSourceCheck(byAppKey)

		byCreatedBy   = "data.sbercloud_apig_applications.filter_by_created_by"
...
data "sbercloud_apig_applications" "test" {
  depends_on = [
    sbercloud_apig_application.test
  ]
...
data "sbercloud_apig_applications" "filter_by_id" {
  instance_id    = local.instance_id
...
locals {
  id_filter_result = [
    for v in data.sbercloud_apig_applications.filter_by_id.applications[*].id : v == local.application_id
  ]
}
...
# Filter by name
locals {
  name = sbercloud_apig_application.test.name
}

data "sbercloud_apig_applications" "filter_by_name" {
  depends_on = [
    sbercloud_apig_application.test
  ]
...
locals {
  name_filter_result = [
    for v in data.sbercloud_apig_applications.filter_by_name.applications[*].name : v == local.name
  ]
}
...
# Filter by app_key
locals {
  app_key = sbercloud_apig_application.test.app_key
}

data "sbercloud_apig_applications" "filter_by_app_key" {
  instance_id = local.instance_id
...
locals {
  app_key_filter_result = [
    for v in data.sbercloud_apig_applications.filter_by_app_key.applications[*].app_key : v == local.app_key
  ]
}
...
locals {
  created_by = data.sbercloud_apig_applications.filter_by_id.applications[0].created_by
}

data "sbercloud_apig_applications" "filter_by_created_by" {
  instance_id = local.instance_id
...
locals {
  created_by_filter_result = [
    for v in data.sbercloud_apig_applications.filter_by_created_by.applications[*].created_by : v == local.created_by
  ]
}

...
func TestAccDataSourceApiBasicConfigurations_basic(t *testing.T) {
	var (
		rName = "data.sbercloud_apig_api_basic_configurations.test"
		dc    = acceptance.InitDataSourceCheck(rName)

		byId   = "data.sbercloud_apig_api_basic_configurations.filter_by_id"
		dcById = acceptance.InitDataSourceCheck(byId)

		byName   = "data.sbercloud_apig_api_basic_configurations.filter_by_name"
		dcByName = acceptance.InitDataSourceCheck(byName)

		byExactName   = "data.sbercloud_apig_api_basic_configurations.filter_by_exact_name"
		dcByExactName = acceptance.InitDataSourceCheck(byExactName)

		byNotFoundName   = "data.sbercloud_apig_api_basic_configurations.filter_by_not_found_name"
		dcByNotFoundName = acceptance.InitDataSourceCheck(byNotFoundName)

		byGroupId   = "data.sbercloud_apig_api_basic_configurations.filter_by_group_id"
		dcByGroupId = acceptance.InitDataSourceCheck(byGroupId)

		byType   = "data.sbercloud_apig_api_basic_configurations.filter_by_type"
		dcByType = acceptance.InitDataSourceCheck(byType)

		byRequestMethod   = "data.sbercloud_apig_api_basic_configurations.filter_by_request_method"
		dcByRequestMethod = acceptance.InitDataSourceCheck(byRequestMethod)

		byRequestPath   = "data.sbercloud_apig_api_basic_configurations.filter_by_request_path"
		dcByRequestPath = acceptance.InitDataSourceCheck(byRequestPath)

		byRequestProtocol   = "data.sbercloud_apig_api_basic_configurations.filter_by_request_protocol"
		dcByRequestProtocol = acceptance.InitDataSourceCheck(byRequestProtocol)

		bySecurityAuthentication   = "data.sbercloud_apig_api_basic_configurations.filter_by_security_authentication"
		dcBySecurityAuthentication = acceptance.InitDataSourceCheck(bySecurityAuthentication)

		byVpcChannelName   = "data.sbercloud_apig_api_basic_configurations.filter_by_vpc_channel_name"
		dcByVpcChannelName = acceptance.InitDataSourceCheck(byVpcChannelName)

		byEnvId   = "data.sbercloud_apig_api_basic_configurations.filter_by_env_id"
		dcByEnvId = acceptance.InitDataSourceCheck(byEnvId)

		byEnvName   = "data.sbercloud_apig_api_basic_configurations.filter_by_env_name"
		dcByEnvName = acceptance.InitDataSourceCheck(byEnvName)

		byBackEndType   = "data.sbercloud_apig_api_basic_configurations.filter_by_backend_type"
...
resource "sbercloud_apig_custom_authorizer" "frontEnd" {
  instance_id      = local.instance_id
  name             = "%[2]s_front"
  function_urn     = sbercloud_fgs_function.test[1].urn
...
resource "sbercloud_apig_api" "test" {
  ...
  matching                = "Exact"
  authorizer_id           = sbercloud_apig_custom_authorizer.frontEnd.id
  cors                    = true
  ...
  web {
    path             = "/"
    vpc_channel_id   = sbercloud_apig_channel.test.id
    request_method   = "GET"
    request_protocol = "HTTP"
    timeout          = 30000
    retry_count      = 1
...
resource "sbercloud_apig_api_publishment" "test" {
  instance_id = local.instance_id
...
data "sbercloud_apig_api_basic_configurations" "test" {
  depends_on = [
    sbercloud_apig_api_publishment.test
  ]
...
data "sbercloud_apig_api_basic_configurations" "filter_by_id" {
  depends_on = [
    sbercloud_apig_api_publishment.test
  ]
...
locals {
  id_filter_result = [
    for v in data.sbercloud_apig_api_basic_configurations.filter_by_id.configurations[*].id : v == local.api_id
  ]
}
...
locals {
  api_name = sbercloud_apig_api.test.name
}

data "sbercloud_apig_api_basic_configurations" "filter_by_name" {
  depends_on = [
    sbercloud_apig_api_publishment.test
  ]
...
locals {
  name_filter_result = [
    for v in data.sbercloud_apig_api_basic_configurations.filter_by_name.configurations[*].name : strcontains(v, local.api_name)
  ]
}
...
data "sbercloud_apig_api_basic_configurations" "filter_by_exact_name" {
  depends_on = [
    sbercloud_apig_api_publishment.test
  ]
...
output "is_exact_name_filter_useful" {
  value = length(data.sbercloud_apig_api_basic_configurations.filter_by_exact_name.configurations) == 1
}
...
data "sbercloud_apig_api_basic_configurations" "filter_by_not_found_name" {
  depends_on = [
    sbercloud_apig_api_publishment.test
  ]
...
output "is_name_not_found_filter_useful" {
  value = length(data.sbercloud_apig_api_basic_configurations.filter_by_not_found_name.configurations) == 0
}

# Filter by group ID
locals {
  group_id = sbercloud_apig_api.test.group_id
}

data "sbercloud_apig_api_basic_configurations" "filter_by_group_id" {
  depends_on = [
    sbercloud_apig_api_publishment.test
  ]
...
locals {
  group_id_filter_result = [
    for v in data.sbercloud_apig_api_basic_configurations.filter_by_id.configurations[*].group_id : v == local.group_id
  ]
}
...
# Filter by type
locals {
  api_type = sbercloud_apig_api.test.type
}

data "sbercloud_apig_api_basic_configurations" "filter_by_type" {
  depends_on = [
    sbercloud_apig_api_publishment.test
  ]
...
locals {
  type_filter_result = [
    for v in data.sbercloud_apig_api_basic_configurations.filter_by_type.configurations[*].type : v == local.api_type
  ]
}
...
locals {
  request_method = sbercloud_apig_api.test.request_method
}

data "sbercloud_apig_api_basic_configurations" "filter_by_request_method" {
  depends_on = [
    sbercloud_apig_api_publishment.test
  ]
...
locals {
  request_method_filter_result = [
    for v in data.sbercloud_apig_api_basic_configurations.filter_by_request_method.configurations[*].request_method : v == local.request_method
  ]
}
...
locals {
  request_path = sbercloud_apig_api.test.request_path
}

data "sbercloud_apig_api_basic_configurations" "filter_by_request_path" {
  depends_on = [
    sbercloud_apig_api_publishment.test
  ]
...
locals {
  request_path_filter_result = [
    for v in data.sbercloud_apig_api_basic_configurations.filter_by_request_path.configurations[*].request_path : v == local.request_path
  ]
}
...
locals {
  request_protocol = sbercloud_apig_api.test.request_protocol
}

data "sbercloud_apig_api_basic_configurations" "filter_by_request_protocol" {
  depends_on = [
    sbercloud_apig_api_publishment.test
  ]
...
locals {
  request_protocol_filter_result = [
    for v in data.sbercloud_apig_api_basic_configurations.filter_by_request_protocol.configurations[*].request_protocol : 
...
locals {
  security_authentication = sbercloud_apig_api.test.security_authentication
}

data "sbercloud_apig_api_basic_configurations" "filter_by_security_authentication" {
  depends_on = [
    sbercloud_apig_api_publishment.test
  ]
...
locals {
  security_auth_filter_result = [
    for v in data.sbercloud_apig_api_basic_configurations.filter_by_security_authentication.configurations[*].security_authentication : 
...
data "sbercloud_apig_api_basic_configurations" "filter_by_vpc_channel_name" {
  depends_on = [
    sbercloud_apig_api_publishment.test
  ]

  instance_id      = local.instance_id
  vpc_channel_name = sbercloud_apig_channel.test.name
}

output "is_vpc_channel_name_filter_useful" {
  value = length(data.sbercloud_apig_api_basic_configurations.filter_by_vpc_channel_name.configurations) > 0
}
...
data "sbercloud_apig_api_basic_configurations" "filter_by_env_id" {
  depends_on = [
    sbercloud_apig_api_publishment.test
  ]
...
locals {
  env_id_filter_result = [
    for v in data.sbercloud_apig_api_basic_configurations.filter_by_env_id.configurations[*].env_id : v == local.env_id
  ]
}
...
# Filter by env name
locals {
  env_name = sbercloud_apig_environment.test.name
}

data "sbercloud_apig_api_basic_configurations" "filter_by_env_name" {
  depends_on = [
    sbercloud_apig_api_publishment.test
  ]
...
locals {
  env_name_filter_result = [
    for v in data.sbercloud_apig_api_basic_configurations.filter_by_env_name.configurations[*].env_name : v == local.env_name
  ]
}
...
locals {
  backend_type = data.sbercloud_apig_api_basic_configurations.filter_by_id.configurations[0].backend_type
}

data "sbercloud_apig_api_basic_configurations" "filter_by_backend_type" {
  depends_on = [
    sbercloud_apig_api_publishment.test
  ]
...
locals {
  backend_type_filter_result = [
    for v in data.sbercloud_apig_api_basic_configurations.filter_by_backend_type.configurations[*].backend_type : v == local.backend_type
  ]
}

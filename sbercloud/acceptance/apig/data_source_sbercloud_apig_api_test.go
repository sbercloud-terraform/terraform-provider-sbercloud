...
func TestAccDataSourceApi_basic(t *testing.T) {
	var (
		...
		webBackend       = "data.sbercloud_apig_api.web"
		dcWithWebBackend = acceptance.InitDataSourceCheck(webBackend)
		fgsBackend       = "data.sbercloud_apig_api.func_graph"
		dcWithFgsBackend = acceptance.InitDataSourceCheck(fgsBackend)
		mockBackend      = "data.sbercloud_apig_api.mock"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(webBackend, "instance_id", "data.sbercloud_apig_instances.test", "instances.0.id"),
					resource.TestCheckResourceAttrPair(webBackend, "api_id", "sbercloud_apig_api.web", "id"),
					resource.TestCheckResourceAttrPair(webBackend, "group_id", "sbercloud_apig_group.test", "id"),
					...
					resource.TestCheckResourceAttrPair(webBackend, "web.0.vpc_channel_id", "sbercloud_apig_channel.test", "id"),
					...
					resource.TestCheckResourceAttrPair(webBackend, "web.0.authorizer_id", "sbercloud_apig_custom_authorizer.test", "id"),
					...
					resource.TestCheckResourceAttrPair(webBackend, "web_policy.0.vpc_channel_id", "sbercloud_apig_channel.test", "id"),
					resource.TestCheckResourceAttrPair(webBackend, "web_policy.0.authorizer_id", "sbercloud_apig_custom_authorizer.test", "id"),
					...
					resource.TestCheckResourceAttrPair(fgsBackend, "instance_id", "data.sbercloud_apig_instances.test", "instances.0.id"),
					resource.TestCheckResourceAttrPair(fgsBackend, "api_id", "sbercloud_apig_api.func_graph", "id"),
					resource.TestCheckResourceAttrPair(fgsBackend, "group_id", "sbercloud_apig_group.test", "id"),
					...
					resource.TestCheckResourceAttrPair(fgsBackend, "func_graph.0.function_urn", "sbercloud_fgs_function.test.1", "urn"),
					resource.TestCheckResourceAttrPair(fgsBackend, "func_graph.0.version", "sbercloud_fgs_function.test.1", "versions.0.name"),
					...
					resource.TestCheckResourceAttrPair(fgsBackend, "func_graph.0.authorizer_id", "sbercloud_apig_custom_authorizer.test", "id"),
					...
					resource.TestCheckResourceAttrPair(fgsBackend, "func_graph_policy.0.function_urn", "sbercloud_fgs_function.test.1", "urn"),
					resource.TestCheckResourceAttrPair(fgsBackend, "func_graph_policy.0.version",
						"sbercloud_fgs_function.test.1", "versions.0.name"),
					...
					resource.TestCheckResourceAttrPair(fgsBackend, "func_graph_policy.0.authorizer_id",
						"sbercloud_apig_custom_authorizer.test", "id"),
					...
					resource.TestCheckResourceAttrPair(mockBackend, "instance_id", "data.sbercloud_apig_instances.test", "instances.0.id"),
					resource.TestCheckResourceAttrPair(mockBackend, "api_id", "sbercloud_apig_api.mock", "id"),
					resource.TestCheckResourceAttrPair(mockBackend, "group_id", "sbercloud_apig_group.test", "id"),
					...
					resource.TestCheckResourceAttrPair(mockBackend, "mock.0.authorizer_id", "sbercloud_apig_custom_authorizer.test", "id"),
					...
					resource.TestCheckResourceAttrPair(mockBackend, "mock_policy.0.authorizer_id", "sbercloud_apig_custom_authorizer.test", "id"),
...
resource "sbercloud_apig_api" "web" {
...
resource "sbercloud_apig_api" "func_graph" {
  instance_id             = local.instance_id
  group_id                = sbercloud_apig_group.test.id
  name                    = "%[2]s_fgs"
  type                    = "Public"
  ...
  func_graph {
    ...
    version          = tolist(sbercloud_fgs_function.test[1].versions)[0].name
    network_type     = "V2"
    request_protocol = "GRPCS"
    timeout          = 5000
...
resource "sbercloud_apig_api" "mock" {
  instance_id             = local.instance_id
  group_id                = sbercloud_apig_group.test.id
  name                    = "%[2]s_mock"
  type                    = "Public"
...
resource "sbercloud_apig_api_publishment" "web" {
  instance_id = local.instance_id
  env_id      = sbercloud_apig_environment.test.id
  api_id      = sbercloud_apig_api.web.id
}

data "sbercloud_apig_api" "web" {
  depends_on = [
    sbercloud_apig_api_publishment.web
  ]
...
resource "sbercloud_apig_api_publishment" "func_graph" {
  instance_id = local.instance_id
  env_id      = sbercloud_apig_environment.test.id
  api_id      = sbercloud_apig_api.func_graph.id
}

data "sbercloud_apig_api" "func_graph" {
  depends_on = [
    sbercloud_apig_api_publishment.func_graph
  ]
...
resource "sbercloud_apig_api_publishment" "mock" {
  instance_id = local.instance_id
  env_id      = sbercloud_apig_environment.test.id
  api_id      = sbercloud_apig_api.mock.id
}

data "sbercloud_apig_api" "mock" {
  depends_on = [
    sbercloud_apig_api_publishment.mock
  ]

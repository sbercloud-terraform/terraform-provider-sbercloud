...
func TestAccDataSourceEndpointConnections_basic(t *testing.T) {
	var (
		dataSource = "data.sbercloud_apig_endpoint_connections.test"
		rName      = acceptance.RandomAccResourceName()
		dc         = acceptance.InitDataSourceCheck(dataSource)

		byEndpointId   = "data.sbercloud_apig_endpoint_connections.by_endpoint_id_filter"
		dcByEndpointId = acceptance.InitDataSourceCheck(byEndpointId)

		byPacketId   = "data.sbercloud_apig_endpoint_connections.by_packet_id_filter"
		dcByPacketId = acceptance.InitDataSourceCheck(byPacketId)

		byStatus   = "data.sbercloud_apig_endpoint_connections.by_status_filter"
...
data "sbercloud_apig_endpoint_connections" "test" {
  depends_on = [
    sbercloud_apig_endpoint_connection_management.test
  ]
...
locals {
  endpoint_id = sbercloud_apig_endpoint_connection_management.test.id
}

data "sbercloud_apig_endpoint_connections" "by_endpoint_id_filter" {
  instance_id = sbercloud_apig_instance.test.id
...
locals {
  endpoint_id_filter_result = [
    for v in data.sbercloud_apig_endpoint_connections.by_endpoint_id_filter.connections[*].id : v == local.endpoint_id
  ]
}
...
locals {
  packet_id = data.sbercloud_apig_endpoint_connections.by_endpoint_id_filter.connections[0].packet_id
}

data "sbercloud_apig_endpoint_connections" "by_packet_id_filter" {
  instance_id = sbercloud_apig_instance.test.id
...
locals {
  packet_id_filter_result = [
    for v in data.sbercloud_apig_endpoint_connections.by_packet_id_filter.connections[*].packet_id : v == local.packet_id
  ]
}
...
# By status filter
locals {
  status = sbercloud_apig_endpoint_connection_management.test.status
}

data "sbercloud_apig_endpoint_connections" "by_status_filter" {
  instance_id = sbercloud_apig_instance.test.id
  status      = local.status
}
...
locals {
  status_filter_result = [
    for v in data.sbercloud_apig_endpoint_connections.by_status_filter.connections[*].status : v == local.status
  ]
}

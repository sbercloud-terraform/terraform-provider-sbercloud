...
func TestAccDataSourceApiAssociatedThrottlingPolicies_basic(t *testing.T) {
	var (
		rName = "data.sbercloud_apig_api_associated_throttling_policies.test"
		dc    = acceptance.InitDataSourceCheck(rName)

		byId   = "data.sbercloud_apig_api_associated_throttling_policies.filter_by_id"
		dcById = acceptance.InitDataSourceCheck(byId)

		byName   = "data.sbercloud_apig_api_associated_throttling_policies.filter_by_name"
		dcByName = acceptance.InitDataSourceCheck(byName)

		byNotFoundName   = "data.sbercloud_apig_api_associated_throttling_policies.filter_by_not_found_name"
		dcByNotFoundName = acceptance.InitDataSourceCheck(byNotFoundName)

		byType   = "data.sbercloud_apig_api_associated_throttling_policies.filter_by_type"
		dcByType = acceptance.InitDataSourceCheck(byType)

		byEnvName   = "data.sbercloud_apig_api_associated_throttling_policies.filter_by_env_name"
		dcByEnvName = acceptance.InitDataSourceCheck(byEnvName)

		byPeriodUnit   = "data.sbercloud_apig_api_associated_throttling_policies.filter_by_period_unit"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "policies.0.user_throttles.0.throttling_object_id",
						"sbercloud_apig_throttling_policy.test", "user_throttles.0.throttling_object_id"),
					resource.TestCheckResourceAttrPair(rName, "policies.0.app_throttles.0.throttling_object_id",
						"sbercloud_apig_throttling_policy.test", "app_throttles.0.throttling_object_id"),
...
resource "sbercloud_apig_channel" "test" {
  ...
  member {
    id   = sbercloud_compute_instance.test.id
...
resource "sbercloud_apig_throttling_policy" "test" {
  instance_id      = local.instance_id
  name             = "%[2]s"
  type             = "API-based"
  period           = 15000
  ...
  app_throttles {
    max_api_requests     = 30
    throttling_object_id = sbercloud_apig_application.test.id
  }
  user_throttles {
    max_api_requests     = 30
    throttling_object_id = data.sbercloud_identity_users.test.users[0].id
  }
}

resource "sbercloud_apig_throttling_policy_associate" "test" {
  instance_id = local.instance_id
  policy_id   = sbercloud_apig_throttling_policy.test.id
  publish_ids = [
    sbercloud_apig_api_publishment.test.publish_id
  ]
}
...
data "sbercloud_apig_api_associated_throttling_policies" "test" {
  depends_on = [
    sbercloud_apig_throttling_policy_associate.test,
  ]
...
data "sbercloud_apig_api_associated_throttling_policies" "filter_by_id" {
  depends_on = [
    sbercloud_apig_throttling_policy_associate.test,
  ]
...
locals {
  id_filter_result = [
    for v in data.sbercloud_apig_api_associated_throttling_policies.filter_by_id.policies[*].id : v == local.policy_id
  ]
}
...
# Filter by name
locals {
  policy_name = sbercloud_apig_throttling_policy.test.name
}

data "sbercloud_apig_api_associated_throttling_policies" "filter_by_name" {
  depends_on = [
    sbercloud_apig_throttling_policy_associate.test,
  ]
...
locals {
  name_filter_result = [
    for v in data.sbercloud_apig_api_associated_throttling_policies.filter_by_name.policies[*].name : v == local.policy_name
  ]
}
...
data "sbercloud_apig_api_associated_throttling_policies" "filter_by_not_found_name" {
  depends_on = [
    sbercloud_apig_throttling_policy_associate.test,
  ]
...
locals {
  not_found_name_filter_result = [
    for v in data.sbercloud_apig_api_associated_throttling_policies.filter_by_not_found_name.policies[*].name : strcontains(v, local.not_found_name)
  ]
}
...
# Filter by type
locals {
  policy_type = sbercloud_apig_throttling_policy.test.type
}

data "sbercloud_apig_api_associated_throttling_policies" "filter_by_type" {
  depends_on = [
    sbercloud_apig_throttling_policy_associate.test,
  ]
...
locals {
  type_filter_result = [
    for v in data.sbercloud_apig_api_associated_throttling_policies.filter_by_type.policies[*].type : v == local.policy_type
  ]
}
...
data "sbercloud_apig_api_associated_throttling_policies" "filter_by_env_name" {
  depends_on = [
    sbercloud_apig_throttling_policy_associate.test,
  ]
...
locals {
  env_name_filter_result = [
    for v in data.sbercloud_apig_api_associated_throttling_policies.filter_by_env_name.policies[*].env_name : v == local.env_name
  ]
}
...
locals {
  period_unit = sbercloud_apig_throttling_policy.test.period_unit
}

data "sbercloud_apig_api_associated_throttling_policies" "filter_by_period_unit" {
  depends_on = [
    sbercloud_apig_throttling_policy_associate.test,
  ]
...
locals {
  period_unit_filter_result = [
    for v in data.sbercloud_apig_api_associated_throttling_policies.filter_by_period_unit.policies[*].period_unit : v == local.period_unit
  ]
}

...
func TestAccChannel_basic(t *testing.T) {
	var (
		...
		rName      = "sbercloud_apig_channel.test"
...
`, acceptance.TestBaseComputeResources(name), acceptance.SBC_APIG_DEDICATED_INSTANCE_ID)
}
...
`, baseConfig, name, acceptance.SBC_APIG_DEDICATED_INSTANCE_USED_SUBNET_ID)
}
...
resource "sbercloud_apig_channel" "test" {
  ...
  dynamic "member" {
    for_each = sbercloud_vpc_eip.test[*].address

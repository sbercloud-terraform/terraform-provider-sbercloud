...
func TestAccGroup_basic(t *testing.T) {
	var (
		...
		rNameBasic = "sbercloud_apig_group.basic"
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(rNameBasic, "instance_id", acceptance.SBC_APIG_DEDICATED_INSTANCE_ID),
...
resource "sbercloud_apig_group" "basic" {
  instance_id = "%[1]s"
  name        = "%[2]s"
  description = var.instance_description != "" ? var.instance_description : null
}
`, acceptance.SBC_APIG_DEDICATED_INSTANCE_ID, name, desc)
}
...
func TestAccGroup_withVariables(t *testing.T) {
	var (
		...
		rNameWithVariables = "sbercloud_apig_group.with_variables"
...
resource "sbercloud_apig_group" "with_variables" {
  instance_id = "%[1]s"
  name        = "%[2]s"

  environment {
    environment_id = sbercloud_apig_environment.test[0].id
  ...
  environment {
    environment_id = sbercloud_apig_environment.test[1].id
...
`, acceptance.SBC_APIG_DEDICATED_INSTANCE_ID, name, offset)
}
...
func TestAccGroup_withUrlDomain(t *testing.T) {
	var (
		...
		rNameWithUrlDomain = "sbercloud_apig_group.with_url_domain"
...
func testAccGroup_withUrlDomain_step1(name string) string {
	return fmt.Sprintf(`
resource "sbercloud_apig_group" "with_url_domain" {
  instance_id = "%[1]s"
  name        = "%[2]s"
...
func TestAccGroup_withDomainAccess(t *testing.T) {
	var (
		...
		rNameWithDomainAccess = "sbercloud_apig_group.with_domain_access"
...
func testAccGroup_withDomainAccess_general(name string, accessEnabled bool) string {
	return fmt.Sprintf(`
resource "sbercloud_apig_group" "with_domain_access" {
  instance_id           = "%[1]s"
  name                  = "%[2]s"
  domain_access_enabled = %v
}
`, acceptance.SBC_APIG_DEDICATED_INSTANCE_ID, name, accessEnabled)
}

...
func TestAccDataSourceApplicationQuotas_basic(t *testing.T) {
	var (
		rName = "data.sbercloud_apig_application_quotas.test"
		dc    = acceptance.InitDataSourceCheck(rName)

		byId   = "data.sbercloud_apig_application_quotas.filter_by_id"
		dcById = acceptance.InitDataSourceCheck(byId)

		byName   = "data.sbercloud_apig_application_quotas.filter_by_name"
		dcByName = acceptance.InitDataSourceCheck(byName)

		byNotFoundName   = "data.sbercloud_apig_application_quotas.filter_by_not_found_name"
...
resource "sbercloud_apig_application_quota" "test" {
...
data "sbercloud_apig_application_quotas" "test" {
  depends_on = [
    sbercloud_apig_application_quota.test,
  ]
...
# Filter by ID
locals {
  quota_id = sbercloud_apig_application_quota.test.id
}

data "sbercloud_apig_application_quotas" "filter_by_id" {
  instance_id = local.instance_id
...
locals {
  id_filter_result = [
    for v in data.sbercloud_apig_application_quotas.filter_by_id.quotas[*].id : v == local.quota_id
  ]
}
...
# Filter by name
locals {
  quota_name = sbercloud_apig_application_quota.test.name
}

data "sbercloud_apig_application_quotas" "filter_by_name" {
  depends_on = [
    sbercloud_apig_application_quota.test,
  ]
...
locals {
  name_filter_result = [
    for v in data.sbercloud_apig_application_quotas.filter_by_name.quotas[*].name : v == local.quota_name
  ]
}
...
data "sbercloud_apig_application_quotas" "filter_by_not_found_name" {
  depends_on = [
    sbercloud_apig_application_quota.test,
  ]
...
output "is_name_not_found_filter_useful" {
  value = length(data.sbercloud_apig_application_quotas.filter_by_not_found_name.quotas) < 1
}

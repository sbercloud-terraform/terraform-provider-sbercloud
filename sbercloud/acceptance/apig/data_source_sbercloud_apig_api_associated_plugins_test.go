...
func TestAccDataSourceApiAssociatedPlugins_basic(t *testing.T) {
	var (
		rName = "data.sbercloud_apig_api_associated_plugins.test"
		dc    = acceptance.InitDataSourceCheck(rName)

		byId   = "data.sbercloud_apig_api_associated_plugins.filter_by_id"
		dcById = acceptance.InitDataSourceCheck(byId)

		byName   = "data.sbercloud_apig_api_associated_plugins.filter_by_name"
		dcByName = acceptance.InitDataSourceCheck(byName)

		byNotFoundName   = "data.sbercloud_apig_api_associated_plugins.filter_by_not_found_name"
		dcByNotFoundName = acceptance.InitDataSourceCheck(byNotFoundName)

		byType   = "data.sbercloud_apig_api_associated_plugins.filter_by_type"
		dcByType = acceptance.InitDataSourceCheck(byType)

		byEnvId   = "data.sbercloud_apig_api_associated_plugins.filter_by_env_id"
		dcByEnvId = acceptance.InitDataSourceCheck(byEnvId)

		byEnvName   = "data.sbercloud_apig_api_associated_plugins.filter_by_env_name"
...
resource "sbercloud_apig_plugin" "test" {
  instance_id = local.instance_id
  name        = "%[3]s_cors"
  type        = "cors"
...
resource "sbercloud_apig_plugin_associate" "test" {
  depends_on = [
  ...
  instance_id = local.instance_id
  plugin_id   = sbercloud_apig_plugin.test.id
  env_id      = sbercloud_apig_environment.test[count.index].id
  api_ids     = [sbercloud_apig_api.test.id]
}
...
data "sbercloud_apig_api_associated_plugins" "test" {
  depends_on = [
    sbercloud_apig_plugin_associate.test,
  ]
...
data "sbercloud_apig_api_associated_plugins" "filter_by_id" {
  depends_on = [
    sbercloud_apig_plugin_associate.test,
  ]
...
locals {
  id_filter_result = [
    for v in data.sbercloud_apig_api_associated_plugins.filter_by_id.plugins[*].id : v == local.plugin_id
  ]
}
...
# Filter by name
locals {
  plugin_name = sbercloud_apig_plugin.test.name
}

data "sbercloud_apig_api_associated_plugins" "filter_by_name" {
  depends_on = [
    sbercloud_apig_plugin_associate.test,
  ]
...
locals {
  name_filter_result = [
    for v in data.sbercloud_apig_api_associated_plugins.filter_by_name.plugins[*].name : v == local.plugin_name
  ]
}
...
data "sbercloud_apig_api_associated_plugins" "filter_by_not_found_name" {
  instance_id = local.instance_id
  api_id      = sbercloud_apig_api.test.id
...
locals {
  not_found_name_filter_result = [
    for v in data.sbercloud_apig_api_associated_plugins.filter_by_not_found_name.plugins[*].name : strcontains(v, local.not_found_name)
  ]
}
...
# Filter by type
locals {
  plugin_type = sbercloud_apig_plugin.test.type
}

data "sbercloud_apig_api_associated_plugins" "filter_by_type" {
  depends_on = [
    sbercloud_apig_plugin_associate.test
  ]
...
locals {
  type_filter_result = [
    for v in data.sbercloud_apig_api_associated_plugins.filter_by_type.plugins[*].type : v == local.plugin_type
  ]
}
...
# Filter by env ID
locals {
  env_id = sbercloud_apig_plugin_associate.test[0].env_id
}

data "sbercloud_apig_api_associated_plugins" "filter_by_env_id" {
  instance_id = local.instance_id
  api_id      = sbercloud_apig_api.test.id
...
locals {
  env_id_filter_result = [
    for v in data.sbercloud_apig_api_associated_plugins.filter_by_env_id.plugins[*].env_id : v == local.env_id
  ]
}
...
data "sbercloud_apig_api_associated_plugins" "filter_by_env_name" {
  depends_on = [
    sbercloud_apig_plugin_associate.test
  ]
...
locals {
  env_name_filter_result = [
    for v in data.sbercloud_apig_api_associated_plugins.filter_by_env_name.plugins[*].env_name : v == local.env_name
  ]
}

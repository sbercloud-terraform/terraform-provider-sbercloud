...
func TestAccDataSourceChannels_basic(t *testing.T) {
	var (
		dataSource = "data.sbercloud_apig_channels.test"
		dc         = acceptance.InitDataSourceCheck(dataSource)
		rName      = acceptance.RandomAccResourceName()

		byId   = "data.sbercloud_apig_channels.filter_by_id"
		dcById = acceptance.InitDataSourceCheck(byId)

		byName   = "data.sbercloud_apig_channels.filter_by_name"
		dcByName = acceptance.InitDataSourceCheck(byName)

		byExactName   = "data.sbercloud_apig_channels.filter_by_exact_name"
		dcByExactName = acceptance.InitDataSourceCheck(byExactName)

		byNotFoundName   = "data.sbercloud_apig_channels.filter_by_not_found_name"
		dcByNotFoundName = acceptance.InitDataSourceCheck(byNotFoundName)

		byMemberGroupId   = "data.sbercloud_apig_channels.filter_by_member_group_id"
		dcByMemberGroupId = acceptance.InitDataSourceCheck(byMemberGroupId)

		byMemberGroupName   = "data.sbercloud_apig_channels.filter_by_member_group_name"
...
`, testAccChannel_base(rName), rName, acceptance.SBC_APIG_DEDICATED_INSTANCE_USED_SUBNET_ID,
...
data "sbercloud_apig_channels" "test" {
  depends_on = [
    sbercloud_apig_channel.test
  ]
...
# Filter by ID
locals {
  channel_id = sbercloud_apig_channel.test.id
}

data "sbercloud_apig_channels" "filter_by_id" {
  depends_on = [
    sbercloud_apig_channel.test
  ]
...
locals {
  id_filter_result = [
    for v in data.sbercloud_apig_channels.filter_by_id.vpc_channels[*].id : v == local.channel_id
  ]
}
...
locals {
  name = sbercloud_apig_channel.test.name
}

data "sbercloud_apig_channels" "filter_by_name" {
  depends_on = [
    sbercloud_apig_channel.test
  ]
...
locals {
  name_filter_result = [
    for v in data.sbercloud_apig_channels.filter_by_name.vpc_channels[*].name : v == local.name
  ]
}
...
data "sbercloud_apig_channels" "filter_by_exact_name" {
  depends_on = [
    sbercloud_apig_channel.test
  ]
...
output "exact_name_filter_is_useful" {
  value = length(data.sbercloud_apig_channels.filter_by_exact_name.vpc_channels) == 1
}
...
data "sbercloud_apig_channels" "filter_by_not_found_name" {
  depends_on = [
    sbercloud_apig_channel.test
  ]
...
output "not_found_name_filter_is_useful" {
  value = length(data.sbercloud_apig_channels.filter_by_not_found_name.vpc_channels) == 0
}
...
locals {
  member_group_id = data.sbercloud_apig_channels.filter_by_id.vpc_channels[0].member_group[0].id
}

data "sbercloud_apig_channels" "filter_by_member_group_id" {
  depends_on = [
    sbercloud_apig_channel.test
  ]
...
locals {
  member_group_id_filter_result = [
    for v in flatten(data.sbercloud_apig_channels.filter_by_member_group_id.vpc_channels[*].member_group[*].id) : v == local.member_group_id
  ]
}
...
locals {
  member_group_name = sbercloud_apig_channel.test.member_group[0].name
}

data "sbercloud_apig_channels" "filter_by_member_group_name" {
  depends_on = [
    sbercloud_apig_channel.test
  ]
...
locals {
  member_group_name_filter_result = [
    for v in data.sbercloud_apig_channels.filter_by_member_group_name.vpc_channels[0].member_group[*].name : v == local.member_group_name
  ]
}

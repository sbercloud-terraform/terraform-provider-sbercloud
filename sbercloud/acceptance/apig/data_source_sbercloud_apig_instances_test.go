...
func TestAccDataSourceInstances_basic(t *testing.T) {
	var (
		dataSource = "data.sbercloud_apig_instances.test"
		dc         = acceptance.InitDataSourceCheck(dataSource)
		rName      = acceptance.RandomAccResourceName()

		byId   = "data.sbercloud_apig_instances.filter_by_id"
		dcById = acceptance.InitDataSourceCheck(byId)

		byName   = "data.sbercloud_apig_instances.filter_by_name"
		dcByName = acceptance.InitDataSourceCheck(byName)

		byStatus   = "data.sbercloud_apig_instances.filter_by_status"
		dcByStatus = acceptance.InitDataSourceCheck(byStatus)

		byEpsId   = "data.sbercloud_apig_instances.filter_by_enterprise_project_id"
...
data "sbercloud_apig_instances" "filter_by_id" {
  instance_id = local.instance_id
}

locals {
  id_filter_result = [
    for v in data.sbercloud_apig_instances.filter_by_id.instances[*].id : v == local.instance_id
  ]
}
...
# Filter by name
locals {
  name = sbercloud_apig_instance.test.name
}

data "sbercloud_apig_instances" "filter_by_name" {
  depends_on = [
    sbercloud_apig_instance.test
  ]
...
locals {
  name_filter_result = [
    for v in data.sbercloud_apig_instances.filter_by_name.instances[*].name : v == local.name
  ]
}
...
# Filter by status
locals {
  status = sbercloud_apig_instance.test.status
}

data "sbercloud_apig_instances" "filter_by_status" {
  status = local.status
}

locals {
  status_filter_result = [
    for v in data.sbercloud_apig_instances.filter_by_status.instances[*].status : v == local.status
  ]
}
...
locals {
  enterprise_project_id = sbercloud_apig_instance.test.enterprise_project_id
}

data "sbercloud_apig_instances" "filter_by_enterprise_project_id" {
  depends_on = [
    sbercloud_apig_instance.test
  ]
...
locals {
  enterprise_project_id_filter_result = [
    for v in data.sbercloud_apig_instances.filter_by_enterprise_project_id.instances[*].enterprise_project_id : v == local.enterprise_project_id
  ]
}

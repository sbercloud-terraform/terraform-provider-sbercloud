...
func TestAccPluginAssociate_basic(t *testing.T) {
	var (
		...
		rName1 = "sbercloud_apig_plugin_associate.cors_bind"
		rName2 = "sbercloud_apig_plugin_associate.http_resp_bind"
		rName3 = "sbercloud_apig_plugin_associate.rate_limit_bind"
		rName4 = "sbercloud_apig_plugin_associate.kafka_log_bind"
		rName5 = "sbercloud_apig_plugin_associate.breaker_bind"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName2, "plugin_id",
						"sbercloud_apig_plugin.http_resp", "id"),
					...
					resource.TestCheckResourceAttrPair(rName3, "plugin_id",
						"sbercloud_apig_plugin.rate_limit", "id"),
					...
					resource.TestCheckResourceAttrPair(rName4, "plugin_id",
						"sbercloud_apig_plugin.kafka_log", "id"),
					...
					resource.TestCheckResourceAttrPair(rName5, "plugin_id",
						"sbercloud_apig_plugin.breaker", "id"),
...
resource "sbercloud_apig_api_publishment" "test" {
  ...
  instance_id = local.instance_id
  api_id      = sbercloud_apig_api.test[count.index].id
...
resource "sbercloud_apig_plugin" "http_resp" {
  instance_id = local.instance_id
  name        = "%[2]s_http_resp"
...
resource "sbercloud_apig_plugin" "kafka_log" {
  ...
  content     = jsonencode(
    {
      "broker_list": [for v in local.connect_addresses: format("%%s:%%d", v, sbercloud_dms_kafka_instance.test.port)],
...
resource "sbercloud_apig_plugin_associate" "cors_bind" {
  depends_on = [sbercloud_apig_api_publishment.test]

  instance_id = local.instance_id
  plugin_id   = local.plugin_ids[0]
  env_id      = sbercloud_apig_environment.test.id
  api_ids     = [sbercloud_apig_api.test[0].id]
}

resource "sbercloud_apig_plugin_associate" "http_resp_bind" {
  depends_on = [sbercloud_apig_api_publishment.test]
...
resource "sbercloud_apig_plugin_associate" "rate_limit_bind" {
  depends_on = [sbercloud_apig_api_publishment.test]
...
resource "sbercloud_apig_plugin_associate" "kafka_log_bind" {
  depends_on = [sbercloud_apig_api_publishment.test]
...
resource "sbercloud_apig_plugin_associate" "breaker_bind" {
  depends_on = [sbercloud_apig_api_publishment.test]
...
resource "sbercloud_apig_plugin_associate" "cors_bind" {
  ...
  plugin_id   = local.plugin_ids[0]
  env_id      = sbercloud_apig_environment.test.id
  api_ids     = [sbercloud_apig_api.test[1].id]
}

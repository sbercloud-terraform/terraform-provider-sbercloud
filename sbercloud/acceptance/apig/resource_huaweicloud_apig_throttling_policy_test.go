...
func TestAccThrottlingPolicy_basic(t *testing.T) {
	var (
		...
		rName = "sbercloud_apig_throttling_policy.test"
		rc    = acceptance.InitResourceCheck(rName, &policy, getThrottlingPolicyFunc)

		rNamePre = "sbercloud_apig_throttling_policy.pre_test"
...
# If you want to test a resource (sbercloud_apig_throttling_policy) that does not contain app-specific throttling
...
resource "sbercloud_apig_throttling_policy" "invalid_type" {
  instance_id      = local.instance_id
...
resource "sbercloud_apig_throttling_policy" "invalid_app_id" {
  instance_id      = local.instance_id
...
resource "sbercloud_apig_throttling_policy" "invalid_user_id" {
  instance_id      = local.instance_id
...
resource "sbercloud_apig_throttling_policy" "pre_test" {
  instance_id       = local.instance_id
  name              = "%[2]s_pre_test"
  type              = "API-based"
  period            = 15000
...
resource "sbercloud_apig_throttling_policy" "test" {
  ...
  dynamic "user_throttles" {
    for_each = slice(data.sbercloud_identity_users.test.users[*].id, 0, 2)
...
resource "sbercloud_apig_throttling_policy" "test" {
  ...
  dynamic "user_throttles" {
    for_each = slice(data.sbercloud_identity_users.test.users[*].id, 1, 3)

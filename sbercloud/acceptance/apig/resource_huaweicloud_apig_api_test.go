...
resource "sbercloud_apig_custom_authorizer" "test" {
  instance_id        = local.instance_id
  name               = "%[2]s"
  function_urn       = sbercloud_fgs_function.test[0].urn
  function_alias_uri = format("%%s:!%%s", sbercloud_fgs_function.test[0].urn, tolist(sbercloud_fgs_function.test[0].versions)[0].aliases[0].name)
  network_type       = "V2"
  type               = "BACKEND"
  is_body_send       = true
  user_data          = "Demo"
...
`, acceptance.TestBaseComputeResources(name), name, acceptance.SBC_FGS_AGENCY_NAME,
...
resource "sbercloud_apig_api" "func_graph" {
  ...
  func_graph {
    function_urn       = sbercloud_fgs_function.test[1].urn
    function_alias_urn = format("%%s:!%%s", sbercloud_fgs_function.test[1].urn,
	  tolist(sbercloud_fgs_function.test[1].versions)[0].aliases[0].name)
    network_type       = "V2"
    request_protocol   = "GRPCS"
    timeout            = 6000
    invocation_type    = "sync"

...
func TestAccDataSourceThrottlingPolicies_basic(t *testing.T) {
	var (
		rName = "data.sbercloud_apig_throttling_policies.test"
		dc    = acceptance.InitDataSourceCheck(rName)

		byId   = "data.sbercloud_apig_throttling_policies.filter_by_id"
		dcById = acceptance.InitDataSourceCheck(byId)

		byName   = "data.sbercloud_apig_throttling_policies.filter_by_name"
		dcByName = acceptance.InitDataSourceCheck(byName)

		byNotFoundName   = "data.sbercloud_apig_throttling_policies.filter_by_not_found_name"
		dcByNotFoundName = acceptance.InitDataSourceCheck(byNotFoundName)

		byType   = "data.sbercloud_apig_throttling_policies.filter_by_type"
...
data "sbercloud_apig_throttling_policies" "test" {
  depends_on = [
...
data "sbercloud_apig_throttling_policies" "filter_by_id" {
  depends_on = [
    sbercloud_apig_throttling_policy_associate.test,
  ]
  instance_id = local.instance_id
...
locals {
  id_filter_result = [
    for v in data.sbercloud_apig_throttling_policies.filter_by_id.policies[*].id : v == local.policy_id
  ]
}
...
data "sbercloud_apig_throttling_policies" "filter_by_name" {
  depends_on = [
    sbercloud_apig_throttling_policy_associate.test,
  ]
  instance_id = local.instance_id
...
locals {
  name_filter_result = [
    for v in data.sbercloud_apig_throttling_policies.filter_by_name.policies[*].name : v == local.policy_name
  ]
}
...
data "sbercloud_apig_throttling_policies" "filter_by_not_found_name" {
  depends_on = [
    sbercloud_apig_throttling_policy_associate.test,
  ]
  instance_id = local.instance_id
...
locals {
  not_found_name_filter_result = [
    for v in data.sbercloud_apig_throttling_policies.filter_by_not_found_name.policies[*].name : strcontains(v, local.not_found_name)
  ]
}
...
data "sbercloud_apig_throttling_policies" "filter_by_type" {
  depends_on = [
    sbercloud_apig_throttling_policy_associate.test,
  ]
  instance_id = local.instance_id
...
locals {
  type_filter_result = [
    for v in data.sbercloud_apig_throttling_policies.filter_by_type.policies[*].type : v == local.policy_type
  ]
}

...
func getApplicationQuotaAssociateFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := cfg.NewServiceClient("apig", acceptance.SBC_REGION_NAME)
	if err != nil {
...
func TestAccApplicationQuotaAssociate_basic(t *testing.T) {
	var (
		...
		rNamePart1 = "sbercloud_apig_application_quota_associate.part1"
		rcPart1    = acceptance.InitResourceCheck(rNamePart1, &obj, getApplicationQuotaAssociateFunc)
		rNamePart2 = "sbercloud_apig_application_quota_associate.part2"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rNamePart1, "quota_id",
						"sbercloud_apig_application_quota.test.0", "id"),
					...
					resource.TestCheckResourceAttrPair(rNamePart2, "quota_id",
						"sbercloud_apig_application_quota.test.1", "id"),
...
resource "sbercloud_apig_application_quota_associate" "part1" {
  instance_id = local.instance_id
  quota_id    = sbercloud_apig_application_quota.test[0].id

  dynamic "applications" {
    for_each = slice(sbercloud_apig_application.test[*].id, 0, 2)
...
resource "sbercloud_apig_application_quota_associate" "part2" {
  instance_id = local.instance_id
  quota_id    = sbercloud_apig_application_quota.test[1].id

  dynamic "applications" {
    for_each = slice(sbercloud_apig_application.test[*].id, 2, 3)
...
resource "sbercloud_apig_application_quota_associate" "part1" {
  ...
  dynamic "applications" {
    for_each = slice(sbercloud_apig_application.test[*].id, 0, 1)
...
resource "sbercloud_apig_application_quota_associate" "part2" {
  ...
  dynamic "applications" {
    for_each = slice(sbercloud_apig_application.test[*].id, 1, 3)

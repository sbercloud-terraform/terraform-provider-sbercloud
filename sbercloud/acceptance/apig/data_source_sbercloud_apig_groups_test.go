...
func TestAccDataSourceGroups_basic(t *testing.T) {
	var (
		all = "data.sbercloud_apig_groups.test"
		dc  = acceptance.InitDataSourceCheck(all)

		byId   = "data.sbercloud_apig_groups.filter_by_id"
		dcById = acceptance.InitDataSourceCheck(byId)

		byName   = "data.sbercloud_apig_groups.filter_by_name"
		dcByName = acceptance.InitDataSourceCheck(byName)

		byNotFoundName   = "data.sbercloud_apig_groups.filter_by_not_found_name"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(byId, "groups.0.id", "sbercloud_apig_group.test", "id"),
					resource.TestCheckResourceAttrPair(byId, "groups.0.name", "sbercloud_apig_group.test", "name"),
					...
					resource.TestCheckResourceAttrPair(byId, "groups.0.environment.0.environment_id", "sbercloud_apig_environment.test", "id"),
...
resource "sbercloud_apig_group" "test" {
  ...
  environment {
    environment_id = sbercloud_apig_environment.test.id
...
data "sbercloud_apig_groups" "test" {
  depends_on = [sbercloud_apig_group.test]
...
data "sbercloud_apig_groups" "filter_by_id" {
  instance_id = local.instance_id
...
locals {
  id_filter_result = [
    for v in data.sbercloud_apig_groups.filter_by_id.groups[*].id : v == local.group_id
  ]
}
...
# Filter by name
locals {
  group_name = sbercloud_apig_group.test.name
}

data "sbercloud_apig_groups" "filter_by_name" {
...
locals {
  name_filter_result = [
    for v in data.sbercloud_apig_groups.filter_by_name.groups[*].name : v == local.group_name
  ]
}
...
data "sbercloud_apig_groups" "filter_by_not_found_name" {
...
output "is_not_found_name_filter_useful" {
  value = length(data.sbercloud_apig_groups.filter_by_not_found_name.groups) == 0
}

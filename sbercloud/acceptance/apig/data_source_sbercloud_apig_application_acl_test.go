...
func TestAccDataSourceApplicationAcl_basic(t *testing.T) {
	var (
		rName = "data.sbercloud_apig_application_acl.test"
		dc    = acceptance.InitDataSourceCheck(rName)

		rNameNotFound = "data.sbercloud_apig_application_acl.not_found"
...
data "sbercloud_apig_application_acl" "test" {
  depends_on = [
    sbercloud_apig_application_acl.test
  ]
...
data "sbercloud_apig_application_acl" "not_found" {
  instance_id    = local.instance_id
...
`, acceptance.SBC_APIG_DEDICATED_INSTANCE_ID, uuid)
}

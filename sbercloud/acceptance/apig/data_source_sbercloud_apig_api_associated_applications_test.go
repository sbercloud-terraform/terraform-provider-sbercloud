...
func TestAccDataSourceApiAssociatedApplications_basic(t *testing.T) {
	var (
		rName = "data.sbercloud_apig_api_associated_applications.test"
		dc    = acceptance.InitDataSourceCheck(rName)

		byId   = "data.sbercloud_apig_api_associated_applications.filter_by_id"
		dcById = acceptance.InitDataSourceCheck(byId)

		byName   = "data.sbercloud_apig_api_associated_applications.filter_by_name"
		dcByName = acceptance.InitDataSourceCheck(byName)

		byNotFoundName   = "data.sbercloud_apig_api_associated_applications.filter_by_not_found_name"
		dcByNotFoundName = acceptance.InitDataSourceCheck(byNotFoundName)

		byEnvId   = "data.sbercloud_apig_api_associated_applications.filter_by_env_id"
		dcByEnvId = acceptance.InitDataSourceCheck(byEnvId)

		byEnvName   = "data.sbercloud_apig_api_associated_applications.filter_by_env_name"
...
resource "sbercloud_apig_application_authorization" "test" {
  depends_on = [sbercloud_apig_api_publishment.test]

  count = 2

  instance_id    = local.instance_id
  application_id = sbercloud_apig_application.test.id
  env_id         = sbercloud_apig_environment.test[count.index].id
...
data "sbercloud_apig_api_associated_applications" "test" {
  depends_on = [
    sbercloud_apig_application_authorization.test,
  ]
...
data "sbercloud_apig_api_associated_applications" "filter_by_id" {
  depends_on = [
    sbercloud_apig_application_authorization.test,
  ]
...
locals {
  id_filter_result = [
    for v in data.sbercloud_apig_api_associated_applications.filter_by_id.applications[*].id : v == local.application_id
  ]
}
...
# Filter by name
locals {
  application_name = sbercloud_apig_application.test.name
}

data "sbercloud_apig_api_associated_applications" "filter_by_name" {
  depends_on = [
    sbercloud_apig_application_authorization.test,
  ]
...
locals {
  name_filter_result = [
    for v in data.sbercloud_apig_api_associated_applications.filter_by_name.applications[*].name : v == local.application_name
  ]
}
...
data "sbercloud_apig_api_associated_applications" "filter_by_not_found_name" {
  depends_on = [
    sbercloud_apig_application_authorization.test,
  ]
...
locals {
  not_found_name_filter_result = [
    for v in data.sbercloud_apig_api_associated_applications.filter_by_not_found_name.applications[*].name : strcontains(v, local.not_found_name)
  ]
}
...
# Filter by env ID
locals {
  env_id = sbercloud_apig_application_authorization.test[0].env_id
}

data "sbercloud_apig_api_associated_applications" "filter_by_env_id" {
  depends_on = [
    sbercloud_apig_application_authorization.test,
  ]
...
locals {
  env_id_filter_result = [
    for v in data.sbercloud_apig_api_associated_applications.filter_by_env_id.applications[*].env_id : v == local.env_id
  ]
}
...
data "sbercloud_apig_api_associated_applications" "filter_by_env_name" {
  depends_on = [
    sbercloud_apig_application_authorization.test
  ]
...
locals {
  env_name_filter_result = [
    for v in data.sbercloud_apig_api_associated_applications.filter_by_env_name.applications[*].env_name : v == local.env_name
  ]
}

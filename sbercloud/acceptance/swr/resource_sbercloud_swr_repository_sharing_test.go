...
func getResourceRepositorySharing(conf *config.Config, state *terraform.ResourceState) (interface{}, error) {
	swrClient, err := conf.SwrV2Client(acceptance.SBC_REGION_NAME)
	if err != nil {
		return nil, fmt.Errorf("Error creating SberCloud SWR client: %s", err)
	}
...
func TestAccSWRRepositorySharing_basic(t *testing.T) {
	...
	resourceName := "sbercloud_swr_repository_sharing.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					acceptance.TestCheckResourceAttrWithVariable(resourceName, "organization",
						"${sbercloud_swr_organization.test.name}"),
					acceptance.TestCheckResourceAttrWithVariable(resourceName, "repository",
						"${sbercloud_swr_repository.test.name}"),
					resource.TestCheckResourceAttr(resourceName, "sharing_account", acceptance.SBC_SWR_SHARING_ACCOUNT),
...
func testAccSWRRepositorySharingImportStateIdFunc() resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		...
		for _, rs := range s.RootModule().Resources {
			if rs.Type == "sbercloud_swr_organization" {
				organization = rs.Primary.Attributes["name"]
			} else if rs.Type == "sbercloud_swr_repository" {
				repositoryID = rs.Primary.ID
			} else if rs.Type == "sbercloud_swr_repository_sharing" {
...
resource "sbercloud_swr_repository_sharing" "test" {
  organization    = sbercloud_swr_organization.test.name
  repository      = sbercloud_swr_repository.test.name
  sharing_account = "%s"
  permission      = "pull"
  deadline        = "forever"
}
`, testAccSWRRepository_basic(rName), acceptance.SBC_SWR_SHARING_ACCOUNT)
}

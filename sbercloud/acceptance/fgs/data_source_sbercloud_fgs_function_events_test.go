...
func TestAccDataFunctionEvents_basic(t *testing.T) {
	var (
		all = "data.sbercloud_fgs_function_events.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(all, "events.0.id", "sbercloud_fgs_function_event.test", "id"),
					resource.TestCheckResourceAttrPair(all, "events.0.name", "sbercloud_fgs_function_event.test", "name"),
...
data "sbercloud_fgs_function_events" "test" {
  depends_on = [
    ...
    sbercloud_fgs_function_event.test
  ]

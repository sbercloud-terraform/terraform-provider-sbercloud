...
func TestAccDataApplications_basic(t *testing.T) {
	var (
		base = "sbercloud_fgs_application.test"

		all          = "data.sbercloud_fgs_applications.all"
		dcForAllApps = acceptance.InitDataSourceCheck(all)

		byAppId           = "data.sbercloud_fgs_applications.filter_by_application_id"
		dcByAppId         = acceptance.InitDataSourceCheck(byAppId)
		byNotFoundAppId   = "data.sbercloud_fgs_applications.filter_by_not_found_application_id"
		dcByNotFoundAppId = acceptance.InitDataSourceCheck(byNotFoundAppId)

		byAppName           = "data.sbercloud_fgs_applications.filter_by_name"
		dcByAppName         = acceptance.InitDataSourceCheck(byAppName)
		byNotFoundAppName   = "data.sbercloud_fgs_applications.filter_by_not_found_name"
		dcByNotFoundAppName = acceptance.InitDataSourceCheck(byNotFoundAppName)

		byAppStatus           = "data.sbercloud_fgs_applications.filter_by_status"
		dcByAppStatus         = acceptance.InitDataSourceCheck(byAppStatus)
		byNotFoundAppStatus   = "data.sbercloud_fgs_applications.filter_by_not_found_status"
		dcByNotFoundAppStatus = acceptance.InitDataSourceCheck(byNotFoundAppStatus)

		byAppDesc           = "data.sbercloud_fgs_applications.filter_by_description"
		dcByAppDesc         = acceptance.InitDataSourceCheck(byAppDesc)
		byNotFoundAppDesc   = "data.sbercloud_fgs_applications.filter_by_not_found_description"
...
data "sbercloud_fgs_applications" "all" {
  # Query applications after application resource create.
  depends_on = [
    sbercloud_fgs_application.test,
  ]
}

# Filter by application ID.
locals {
  application_id = sbercloud_fgs_application.test.id
}

data "sbercloud_fgs_applications" "filter_by_application_id" {
  application_id = local.application_id
}

data "sbercloud_fgs_applications" "filter_by_not_found_application_id" {
...
locals {
  application_id_filter_result = [for v in data.sbercloud_fgs_applications.filter_by_application_id.applications[*].id :
...
output "application_id_not_found_validation_pass" {
  value = length(data.sbercloud_fgs_applications.filter_by_not_found_application_id.applications) == 0
}

# Filter by application name.
locals {
  application_name = sbercloud_fgs_application.test.name
}

data "sbercloud_fgs_applications" "filter_by_name" {
...
data "sbercloud_fgs_applications" "filter_by_not_found_name" {
...
locals {
  name_filter_result = [for v in data.sbercloud_fgs_applications.filter_by_name.applications[*].name :
...
output "name_not_found_validation_pass" {
  value = length(data.sbercloud_fgs_applications.filter_by_not_found_name.applications) == 0
}

# Filter by application status.
locals {
  application_status = sbercloud_fgs_application.test.status
}

data "sbercloud_fgs_applications" "filter_by_status" {
  status = local.application_status
}

data "sbercloud_fgs_applications" "filter_by_not_found_status" {
...
locals {
  status_filter_result = [for v in data.sbercloud_fgs_applications.filter_by_status.applications[*].status :
...
output "status_not_found_validation_pass" {
  value = length(data.sbercloud_fgs_applications.filter_by_not_found_status.applications) == 0
}

# Filter by application description.
locals {
  application_description = sbercloud_fgs_application.test.description
}

data "sbercloud_fgs_applications" "filter_by_description" {
...
data "sbercloud_fgs_applications" "filter_by_not_found_description" {
...
locals {
  description_filter_result = [for v in data.sbercloud_fgs_applications.filter_by_description.applications[*].description :
...
output "description_not_found_validation_pass" {
  value = length(data.sbercloud_fgs_applications.filter_by_not_found_description.applications) == 0
}

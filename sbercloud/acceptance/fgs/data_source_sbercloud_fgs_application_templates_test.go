...
func TestAccDataApplicationTemplates_basic(t *testing.T) {
	var (
		all               = "data.sbercloud_fgs_application_templates.all"
		dcForAllTemplates = acceptance.InitDataSourceCheck(all)

		byRuntime           = "data.sbercloud_fgs_application_templates.filter_by_runtime"
		dcByRuntime         = acceptance.InitDataSourceCheck(byRuntime)
		byNotFoundRuntime   = "data.sbercloud_fgs_application_templates.filter_by_not_found_runtime"
		dcByNotFoundRuntime = acceptance.InitDataSourceCheck(byNotFoundRuntime)

		byCategory           = "data.sbercloud_fgs_application_templates.filter_by_category"
		dcByCategory         = acceptance.InitDataSourceCheck(byCategory)
		byNotFoundCategory   = "data.sbercloud_fgs_application_templates.filter_by_not_found_category"
...
data "sbercloud_fgs_application_templates" "all" {}

# Filter by application runtime.
locals {
  application_runtime = data.sbercloud_fgs_application_templates.all.templates[0].runtime
}

data "sbercloud_fgs_application_templates" "filter_by_runtime" {
  runtime = local.application_runtime
}

data "sbercloud_fgs_application_templates" "filter_by_not_found_runtime" {
  runtime = "runtime_not_found"
}

locals {
  runtime_filter_result = [for v in data.sbercloud_fgs_application_templates.filter_by_runtime.templates[*].runtime :
...
output "runtime_not_found_validation_pass" {
  value = length(data.sbercloud_fgs_application_templates.filter_by_not_found_runtime.templates) == 0
}

# Filter by application category.
locals {
  application_category = data.sbercloud_fgs_application_templates.all.templates[0].category
}

data "sbercloud_fgs_application_templates" "filter_by_category" {
  category = local.application_category
}

data "sbercloud_fgs_application_templates" "filter_by_not_found_category" {
  category = "category_not_found"
}

locals {
  category_filter_result = [for v in data.sbercloud_fgs_application_templates.filter_by_category.templates[*].category :
...
output "category_not_found_validation_pass" {
  value = length(data.sbercloud_fgs_application_templates.filter_by_not_found_category.templates) == 0
}
`

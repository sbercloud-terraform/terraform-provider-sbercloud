...
func TestAccApplication_basic(t *testing.T) {
	var (
		...
		resourceName = "sbercloud_fgs_application.test"
...
func testAccApplication_invalidTemplateId(randUUID string) string {
	return fmt.Sprintf(`
resource "sbercloud_fgs_application" "invalid_template_id" {
  name        = "tf_test_invalid_template_id"
  agency_name = "%[1]s"
  template_id = "%[2]s"
  description = "The template ID is invalid"
}
`, acceptance.SBC_FGS_AGENCY_NAME, randUUID)
}
...
resource "sbercloud_fgs_application" "test" {
  name        = "%[1]s"
  agency_name = "%[2]s"
  template_id = data.sbercloud_fgs_application_templates.test.templates[0].id

...
func TestAccFunctionTrigger_basic(t *testing.T) {
	var (
		...
		resNameFunc      = "sbercloud_fgs_function.test"
		resNameTimerRate = "sbercloud_fgs_function_trigger.timer_rate"
		resNameTimerCron = "sbercloud_fgs_function_trigger.timer_cron"
...
// Timing trigger (with rate schedule type)
resource "sbercloud_fgs_function_trigger" "timer_rate" {
...
lifecycle {
    ignore_changes = [event_data]  # Игнорируем изменения в event_data
  }
}

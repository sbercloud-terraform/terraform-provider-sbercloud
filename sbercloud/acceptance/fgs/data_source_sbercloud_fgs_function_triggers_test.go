...
func TestAccDataFunctionTriggers_basic(t *testing.T) {
	var (
		base = "sbercloud_fgs_function_trigger.test"

		all              = "data.sbercloud_fgs_function_triggers.all"
		dcForAllTriggers = acceptance.InitDataSourceCheck(all)

		byTriggerId           = "data.sbercloud_fgs_function_triggers.filter_by_trigger_id"
		dcByTriggerId         = acceptance.InitDataSourceCheck(byTriggerId)
		byNotFoundTriggerId   = "data.sbercloud_fgs_function_triggers.filter_by_not_found_trigger_id"
		dcByNotFoundTriggerId = acceptance.InitDataSourceCheck(byNotFoundTriggerId)

		byType           = "data.sbercloud_fgs_function_triggers.filter_by_type"
		dcByType         = acceptance.InitDataSourceCheck(byType)
		byNotFoundType   = "data.sbercloud_fgs_function_triggers.filter_by_not_found_type"
		dcByNotFoundType = acceptance.InitDataSourceCheck(byNotFoundType)

		byStatus           = "data.sbercloud_fgs_function_triggers.filter_by_status"
		dcByStatus         = acceptance.InitDataSourceCheck(byStatus)
		byNotFoundStatus   = "data.sbercloud_fgs_function_triggers.filter_by_not_found_status"
		dcByNotFoundStatus = acceptance.InitDataSourceCheck(byNotFoundStatus)

		byStartTime   = "data.sbercloud_fgs_function_triggers.filter_by_start_time"
		dcByStartTime = acceptance.InitDataSourceCheck(byStartTime)
		byEndTime     = "data.sbercloud_fgs_function_triggers.filter_by_end_time"
...
data "sbercloud_fgs_function_triggers" "all" {
  depends_on = [
    sbercloud_fgs_function_trigger.test
  ]
...
# Filter by trigger ID.
locals {
  trigger_id = sbercloud_fgs_function_trigger.test.id
}

data "sbercloud_fgs_function_triggers" "filter_by_trigger_id" {
  function_urn = sbercloud_fgs_function.test.urn
  trigger_id   = local.trigger_id
}

data "sbercloud_fgs_function_triggers" "filter_by_not_found_trigger_id" {
...
locals {
  trigger_id_filter_result = [
    for v in data.sbercloud_fgs_function_triggers.filter_by_trigger_id.triggers[*].id : v == local.trigger_id
  ]
}
...
output "trigger_id_not_found_validation_pass" {
  value = length(data.sbercloud_fgs_function_triggers.filter_by_not_found_trigger_id.triggers) == 0
}

# Filter by trigger type.
locals {
  trigger_type = sbercloud_fgs_function_trigger.test.type
}

data "sbercloud_fgs_function_triggers" "filter_by_type" {
...
data "sbercloud_fgs_function_triggers" "filter_by_not_found_type" {
...
locals {
  type_filter_result = [
    for v in data.sbercloud_fgs_function_triggers.filter_by_type.triggers[*].type : v == local.trigger_type
  ]
}
...
output "type_not_found_validation_pass" {
  value = length(data.sbercloud_fgs_function_triggers.filter_by_not_found_type.triggers) == 0
}

# Filter by trigger status.
locals {
  trigger_status = sbercloud_fgs_function_trigger.test.status
}

data "sbercloud_fgs_function_triggers" "filter_by_status" {
  function_urn = sbercloud_fgs_function.test.urn
  status       = local.trigger_status
}

data "sbercloud_fgs_function_triggers" "filter_by_not_found_status" {
  function_urn = sbercloud_fgs_function.test.urn
...
locals {
  status_filter_result = [
    for v in data.sbercloud_fgs_function_triggers.filter_by_status.triggers[*].status : v == local.trigger_status
  ]
}
...
output "status_not_found_validation_pass" {
  value = length(data.sbercloud_fgs_function_triggers.filter_by_not_found_status.triggers) == 0
}

# Filter by start time
locals {
  start_time = sbercloud_fgs_function_trigger.test.created_at
}

data "sbercloud_fgs_function_triggers" "filter_by_start_time" {
  function_urn = sbercloud_fgs_function.test.urn
...
locals {
  start_time_filter_result = [
    for v in data.sbercloud_fgs_function_triggers.filter_by_start_time.triggers[*].created_at : timecmp(v, local.start_time) >= 0
  ]
}
...
# Filter by end time
locals {
  end_time = sbercloud_fgs_function_trigger.test.created_at
}

data "sbercloud_fgs_function_triggers" "filter_by_end_time" {
  function_urn = sbercloud_fgs_function.test.urn
...
locals {
  end_time_filter_result = [
    for v in data.sbercloud_fgs_function_triggers.filter_by_end_time.triggers[*].created_at : timecmp(v, local.end_time) <= 0
  ]
}

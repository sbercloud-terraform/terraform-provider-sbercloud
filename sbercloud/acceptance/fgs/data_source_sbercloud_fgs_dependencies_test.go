...
func TestAccDataDependencies_basic(t *testing.T) {
	var (
		...
		all                  = "data.sbercloud_fgs_dependencies.all"
		dcForAllDependencies = acceptance.InitDataSourceCheck(all)

		byType   = "data.sbercloud_fgs_dependencies.filter_by_type"
		dcByType = acceptance.InitDataSourceCheck(byType)

		byName                    = "data.sbercloud_fgs_dependencies.filter_by_name"
		dcByName                  = acceptance.InitDataSourceCheck(byName)
		byNotFoundName            = "data.sbercloud_fgs_dependencies.filter_by_not_found_name"
		dcByNotFoundName          = acceptance.InitDataSourceCheck(byNotFoundName)
		byNameWithVersionsQuery   = "data.sbercloud_fgs_dependencies.filter_by_name_with_versions_query"
		dcByNameWithVersionsQuery = acceptance.InitDataSourceCheck(byNameWithVersionsQuery)

		byRuntime   = "data.sbercloud_fgs_dependencies.filter_by_runtime"
...
data "sbercloud_fgs_dependencies" "invalid_runtime" {
...
data "sbercloud_fgs_dependencies" "all" {}
...
data "sbercloud_fgs_dependencies" "filter_by_type" {
  type = local.dependency_type
}

output "is_type_filter_useful" {
  value = length(data.sbercloud_fgs_dependencies.filter_by_type.packages) > 0
}

# Filter by dependency name.
locals {
  dependency_name = sbercloud_fgs_dependency_version.test.name
}

data "sbercloud_fgs_dependencies" "filter_by_name" {
...
data "sbercloud_fgs_dependencies" "filter_by_not_found_name" {
  name = "name_not_found"
}

data "sbercloud_fgs_dependencies" "filter_by_name_with_versions_query" {
  # The behavior of parameter 'name' of the application resource is 'Required', means this parameter does not
  # have 'Know After Apply' behavior.
  depends_on = [
    sbercloud_fgs_dependency_version.test,
  ...
  name                      = sbercloud_fgs_dependency_version.test.name
  is_versions_query_allowed = true
}

locals {
  name_filter_result = [for v in data.sbercloud_fgs_dependencies.filter_by_name.packages[*].name :
...
output "name_not_found_validation_pass" {
  value = length(data.sbercloud_fgs_dependencies.filter_by_not_found_name.packages) == 0
}

locals {
  name_filter_with_versions_query_result = data.sbercloud_fgs_dependencies.filter_by_name_with_versions_query.packages[0].versions
}
...
# Filter by dependency runtime.
locals {
  dependency_runtime = data.sbercloud_fgs_dependencies.all.packages[0].runtime
}

data "sbercloud_fgs_dependencies" "filter_by_runtime" {
  runtime = local.dependency_runtime
}

locals {
  runtime_filter_result = [for v in data.sbercloud_fgs_dependencies.filter_by_runtime.packages[*].runtime :

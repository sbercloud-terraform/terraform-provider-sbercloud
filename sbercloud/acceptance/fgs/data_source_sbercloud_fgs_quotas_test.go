...
func TestAccDataQuotas_basic(t *testing.T) {
	var (
		all = "data.sbercloud_fgs_quotas.all"
...
data "sbercloud_fgs_quotas" "all" {
  depends_on = [sbercloud_fgs_function.test]
}

locals {
  all_used_quota_types = [for _, v in data.sbercloud_fgs_quotas.all.quotas: v.type if v.used > 0]
}
...
locals {
  all_quotas_that_contains_unit = [for _, v in data.sbercloud_fgs_quotas.all.quotas: v if v.unit != ""]
}

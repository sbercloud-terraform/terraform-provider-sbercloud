...
func TestAccDataFunctions_basic(t *testing.T) {
	var (
		base = "sbercloud_fgs_function.test"

		all               = "data.sbercloud_fgs_functions.all"
		dcForAllFunctions = acceptance.InitDataSourceCheck(all)

		byPackageName           = "data.sbercloud_fgs_functions.filter_by_package_name"
		dcByPackageName         = acceptance.InitDataSourceCheck(byPackageName)
		byNotFoundPackageName   = "data.sbercloud_fgs_functions.filter_by_not_found_package_name"
		dcByNotFoundPackageName = acceptance.InitDataSourceCheck(byNotFoundPackageName)

		byFunctionUrn           = "data.sbercloud_fgs_functions.filter_by_function_urn"
		dcByFunctionUrn         = acceptance.InitDataSourceCheck(byFunctionUrn)
		byNotFoundFunctionUrn   = "data.sbercloud_fgs_functions.filter_by_not_found_function_urn"
		dcByNotFoundFunctionUrn = acceptance.InitDataSourceCheck(byNotFoundFunctionUrn)

		byFunctionName           = "data.sbercloud_fgs_functions.filter_by_name"
		dcByFunctionName         = acceptance.InitDataSourceCheck(byFunctionName)
		byNotFoundFunctionName   = "data.sbercloud_fgs_functions.filter_by_not_found_name"
		dcByNotFoundFunctionName = acceptance.InitDataSourceCheck(byNotFoundFunctionName)

		byRuntime           = "data.sbercloud_fgs_functions.filter_by_runtime"
		dcByRuntime         = acceptance.InitDataSourceCheck(byRuntime)
		byNotFoundRuntime   = "data.sbercloud_fgs_functions.filter_by_not_found_runtime"
		dcByNotFoundRuntime = acceptance.InitDataSourceCheck(byNotFoundRuntime)

		byEpsId            = "data.sbercloud_fgs_functions.filter_by_eps_id"
		dcByEpsId          = acceptance.InitDataSourceCheck(byEpsId)
		byNotFoundEpsId    = "data.sbercloud_fgs_functions.filter_by_not_found_eps_id"
...
data "sbercloud_fgs_functions" "all" {
  depends_on = [
    sbercloud_fgs_function.test
  ]
}

# Filter by package name.
locals {
  package_name = sbercloud_fgs_function.test.package
}

data "sbercloud_fgs_functions" "filter_by_package_name" {
...
data "sbercloud_fgs_functions" "filter_by_not_found_package_name" {
...
locals {
  package_name_filter_result = [for v in data.sbercloud_fgs_functions.filter_by_package_name.functions[*].package :
...
output "package_name_not_found_validation_pass" {
  value = length(data.sbercloud_fgs_functions.filter_by_not_found_package_name.functions) == 0
}
...
data "sbercloud_fgs_functions" "filter_by_function_urn" {
  urn = local.function_urn
}

data "sbercloud_fgs_functions" "filter_by_not_found_function_urn" {
...
locals {
  function_urn_filter_result = [for v in data.sbercloud_fgs_functions.filter_by_function_urn.functions[*].urn :
...
output "function_urn_not_found_validation_pass" {
  value = length(data.sbercloud_fgs_functions.filter_by_not_found_function_urn.functions) == 0
}

# Filter by function name.
locals {
  function_name = sbercloud_fgs_function.test.name
}

data "sbercloud_fgs_functions" "filter_by_name" {
...
data "sbercloud_fgs_functions" "filter_by_not_found_name" {
...
locals {
  name_filter_result = [for v in data.sbercloud_fgs_functions.filter_by_function_urn.functions[*].name :
...
output "name_not_found_validation_pass" {
  value = length(data.sbercloud_fgs_functions.filter_by_not_found_name.functions) == 0
}

# Filter by function runtime.
locals {
  function_runtime = sbercloud_fgs_function.test.runtime
}

data "sbercloud_fgs_functions" "filter_by_runtime" {
...
data "sbercloud_fgs_functions" "filter_by_not_found_runtime" {
...
locals {
  runtime_filter_result = [for v in data.sbercloud_fgs_functions.filter_by_runtime.functions[*].runtime :
...
output "runtime_not_found_validation_pass" {
  value = length(data.sbercloud_fgs_functions.filter_by_not_found_runtime.functions) == 0
}

# Filter by function enterprise project ID.
locals {
  enterprise_project_id = sbercloud_fgs_function.test.enterprise_project_id
}

data "sbercloud_fgs_functions" "filter_by_eps_id" {
  enterprise_project_id = local.enterprise_project_id
}

data "sbercloud_fgs_functions" "filter_by_not_found_eps_id" {
...
locals {
  eps_id_filter_result = [for v in data.sbercloud_fgs_functions.filter_by_function_urn.functions[*].enterprise_project_id :
...
output "eps_id_not_found_validation_pass" {
  value = length(data.sbercloud_fgs_functions.filter_by_not_found_eps_id.functions) == 0
}

...
func TestAccDataDependencyVersions_basic(t *testing.T) {
	var (
		base = "sbercloud_fgs_dependency_version.test"

		all                        = "data.sbercloud_fgs_dependency_versions.all"
		dcForAllDependencyVersions = acceptance.InitDataSourceCheck(all)

		byVersionId           = "data.sbercloud_fgs_dependency_versions.filter_by_version_id"
		dcByVersionId         = acceptance.InitDataSourceCheck(byVersionId)
		byNotFoundVersionId   = "data.sbercloud_fgs_dependency_versions.filter_by_not_found_version_id"
		dcByNotFoundVersionId = acceptance.InitDataSourceCheck(byNotFoundVersionId)

		byVersion           = "data.sbercloud_fgs_dependency_versions.filter_by_version"
		dcByVersion         = acceptance.InitDataSourceCheck(byVersion)
		byNotFoundVersion   = "data.sbercloud_fgs_dependency_versions.filter_by_not_found_version"
		dcByNotFoundVersion = acceptance.InitDataSourceCheck(byNotFoundVersion)

		byRuntime           = "data.sbercloud_fgs_dependency_versions.filter_by_runtime"
		dcByRuntime         = acceptance.InitDataSourceCheck(byRuntime)
		byNotFoundRuntime   = "data.sbercloud_fgs_dependency_versions.filter_by_not_found_runtime"
...
data "sbercloud_fgs_dependency_versions" "all" {
  dependency_id = sbercloud_fgs_dependency_version.test.dependency_id
}

# Filter by version ID.
locals {
  version_id = sbercloud_fgs_dependency_version.test.version_id
}

data "sbercloud_fgs_dependency_versions" "filter_by_version_id" {
  dependency_id = sbercloud_fgs_dependency_version.test.dependency_id
  version_id    = local.version_id
}

data "sbercloud_fgs_dependency_versions" "filter_by_not_found_version_id" {
  # Query dependency versions using a not exist version ID after dependency version resource create.
  depends_on = [
    sbercloud_fgs_dependency_version.test,
  ]
...
locals {
  version_id_filter_result = [for v in data.sbercloud_fgs_dependency_versions.filter_by_version_id.versions[*].id :
    v == local.version_id]
}
...
output "version_id_not_found_validation_pass" {
  value = length(data.sbercloud_fgs_dependency_versions.filter_by_not_found_version_id.versions) == 0
}

# Filter by version.
locals {
  version = sbercloud_fgs_dependency_version.test.version
}

data "sbercloud_fgs_dependency_versions" "filter_by_version" {
  dependency_id = sbercloud_fgs_dependency_version.test.dependency_id
  version       = local.version
}

data "sbercloud_fgs_dependency_versions" "filter_by_not_found_version" {
...
locals {
  version_filter_result = [for v in data.sbercloud_fgs_dependency_versions.filter_by_version.versions[*].version :
    v == local.version]
}
...
output "version_not_found_validation_pass" {
  value = length(data.sbercloud_fgs_dependency_versions.filter_by_not_found_version.versions) == 0
}

# Filter by runtime.
locals {
  version_runtime = sbercloud_fgs_dependency_version.test.runtime
}

data "sbercloud_fgs_dependency_versions" "filter_by_runtime" {
...
data "sbercloud_fgs_dependency_versions" "filter_by_not_found_runtime" {
...
locals {
  runtime_filter_result = [for v in data.sbercloud_fgs_dependency_versions.filter_by_runtime.versions[*].runtime :
...
output "runtime_not_found_validation_pass" {
  value = length(data.sbercloud_fgs_dependency_versions.filter_by_not_found_runtime.versions) == 0
}

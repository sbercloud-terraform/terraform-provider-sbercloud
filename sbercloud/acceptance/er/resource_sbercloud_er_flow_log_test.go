...
func TestAccFlowLog_basic(t *testing.T) {
	var (
		flowLog interface{}
		rName   = "sbercloud_er_flow_log.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "log_group_id", "sbercloud_lts_group.test", "id"),
					resource.TestCheckResourceAttrPair(rName, "log_stream_id", "sbercloud_lts_stream.test", "id"),
					resource.TestCheckResourceAttr(rName, "resource_type", "attachment"),
					resource.TestCheckResourceAttrPair(rName, "resource_id", "sbercloud_er_vpc_attachment.test", "id"),
...
resource "sbercloud_er_flow_log" "test" {
  ...
  log_stream_id  = sbercloud_lts_stream.test.id
  resource_type  = "attachment"
  resource_id    = sbercloud_er_vpc_attachment.test.id
  name           = "%[2]s"

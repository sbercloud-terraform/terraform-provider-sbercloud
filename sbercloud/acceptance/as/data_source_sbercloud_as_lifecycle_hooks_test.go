...
func TestAccDataSourceLifecycleHooks_basic(t *testing.T) {
	var (
		dataSourceName = "data.sbercloud_as_lifecycle_hooks.test"
		dc             = acceptance.InitDataSourceCheck(dataSourceName)

		byName   = "data.sbercloud_as_lifecycle_hooks.filter_by_name"
		dcByName = acceptance.InitDataSourceCheck(byName)

		byType   = "data.sbercloud_as_lifecycle_hooks.filter_by_type"
		dcByType = acceptance.InitDataSourceCheck(byType)

		byDefaultResult   = "data.sbercloud_as_lifecycle_hooks.filter_by_default_result"
...
func testAccLifecycleHooks_basic() string {
	return fmt.Sprintf(`
data "sbercloud_as_lifecycle_hooks" "test" {
  scaling_group_id = "%[1]s"
}

# Filter by name
locals {
  name = data.sbercloud_as_lifecycle_hooks.test.lifecycle_hooks[0].name
}

data "sbercloud_as_lifecycle_hooks" "filter_by_name" {
  scaling_group_id = "%[1]s"
  name             = local.name
}
...
locals {
  name_filter_result = [
    for v in data.sbercloud_as_lifecycle_hooks.filter_by_name.lifecycle_hooks[*].name : v == local.name
  ]
}
...
# Filter by type
locals {
  type = data.sbercloud_as_lifecycle_hooks.test.lifecycle_hooks[0].type
}

data "sbercloud_as_lifecycle_hooks" "filter_by_type" {
  scaling_group_id = "%[1]s"
  type             = local.type
}
...
locals {
  type_filter_result = [
    for v in data.sbercloud_as_lifecycle_hooks.filter_by_type.lifecycle_hooks[*].type : v == local.type
  ]
}
...
locals {
  default_result = data.sbercloud_as_lifecycle_hooks.test.lifecycle_hooks[0].default_result
}

data "sbercloud_as_lifecycle_hooks" "filter_by_default_result" {
  scaling_group_id = "%[1]s"
...
locals {
  default_result_filter_result = [
    for v in data.sbercloud_as_lifecycle_hooks.filter_by_default_result.lifecycle_hooks[*].default_result : v == local.default_result
  ]
}

...
func TestAccDataSourceHookInstances_basic(t *testing.T) {
	var (
		dataSource = "data.sbercloud_as_hook_instances.test"
		dc         = acceptance.InitDataSourceCheck(dataSource)

		byInstanceId   = "data.sbercloud_as_hook_instances.filter_by_instance_id"
		dcByInstanceId = acceptance.InitDataSourceCheck(byInstanceId)

		byLifecycleHookName   = "data.sbercloud_as_hook_instances.filter_by_name"
		dcByLifecycleHookName = acceptance.InitDataSourceCheck(byLifecycleHookName)

		byLifecycleHookStatus   = "data.sbercloud_as_hook_instances.filter_by_status"
...
func testDataSourceHookInstances_basic() string {
	return fmt.Sprintf(`
data "sbercloud_as_hook_instances" "test" {
  scaling_group_id = "%[1]s"
}

locals {
  instance_id = data.sbercloud_as_hook_instances.test.instance_hanging_info[0].instance_id
}

data "sbercloud_as_hook_instances" "filter_by_instance_id" {
  scaling_group_id = "%[1]s"
...
locals {
  instance_id_filter_result = [
    for v in data.sbercloud_as_hook_instances.filter_by_instance_id.instance_hanging_info[*].instance_id : 
    v == local.instance_id
  ]
}
...
locals {
  lifecycle_hook_name = data.sbercloud_as_hook_instances.test.instance_hanging_info[0].lifecycle_hook_name
}

data "sbercloud_as_hook_instances" "filter_by_name" {
  scaling_group_id    = "%[1]s"
...
locals {
  lifecycle_hook_name_filter_result = [
    for v in data.sbercloud_as_hook_instances.filter_by_name.instance_hanging_info[*].lifecycle_hook_name : 
...
locals {
  lifecycle_hook_status = data.sbercloud_as_hook_instances.test.instance_hanging_info[0].lifecycle_hook_status
}

data "sbercloud_as_hook_instances" "filter_by_status" {
  scaling_group_id      = "%[1]s"
...
locals {
  lifecycle_hook_status_filter_result = [
    for v in data.sbercloud_as_hook_instances.filter_by_status.instance_hanging_info[*].lifecycle_hook_status : 

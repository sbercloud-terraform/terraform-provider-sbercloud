...
func TestAccDatasourceASInstances_basic(t *testing.T) {
	var (
		rName = "data.sbercloud_as_instances.test"
		dc    = acceptance.InitDataSourceCheck(rName)

		byLifeCycleState   = "data.sbercloud_as_instances.life_cycle_state_filter"
		dcByLifeCycleState = acceptance.InitDataSourceCheck(byLifeCycleState)

		byHealthStatus   = "data.sbercloud_as_instances.health_status_filter"
		dcByHealthStatus = acceptance.InitDataSourceCheck(byHealthStatus)

		byProtectFromScalingDown   = "data.sbercloud_as_instances.protect_from_scaling_down_filter"
...
func testAccDatasourceASInstances_basic() string {
	return fmt.Sprintf(`
data "sbercloud_as_instances" "test" {
  scaling_group_id = "%[1]s"
}

# Test with life_cycle_state
locals {
  life_cycle_state = data.sbercloud_as_instances.test.instances[0].life_cycle_state
}

data "sbercloud_as_instances" "life_cycle_state_filter" {
  scaling_group_id = "%[1]s"
...
locals {
  life_cycle_state_filter_result = [
    for v in data.sbercloud_as_instances.life_cycle_state_filter.instances[*].life_cycle_state : v == local.life_cycle_state
  ]
}
...
locals {
  health_status = data.sbercloud_as_instances.test.instances[0].health_status
}

data "sbercloud_as_instances" "health_status_filter" {
  scaling_group_id = "%[1]s"
...
locals {
  health_status_filter_result = [
    for v in data.sbercloud_as_instances.health_status_filter.instances[*].health_status : v == local.health_status
  ]
}
...
locals {
  protect_from_scaling_down = data.sbercloud_as_instances.test.instances[0].protect_from_scaling_down
}

data "sbercloud_as_instances" "protect_from_scaling_down_filter" {
  scaling_group_id          = "%[1]s"
...
locals {
  protect_from_scaling_down_filter_result = [
    for v in data.sbercloud_as_instances.protect_from_scaling_down_filter.instances[*].protect_from_scaling_down :

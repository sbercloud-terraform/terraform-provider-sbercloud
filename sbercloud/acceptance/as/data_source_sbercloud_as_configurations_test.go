...
func TestAccDataSourceASConfiguration_basic(t *testing.T) {
	var (
		dataSourceName = "data.sbercloud_as_configurations.test"
		dc             = acceptance.InitDataSourceCheck(dataSourceName)

		byName   = "data.sbercloud_as_configurations.name_filter"
		dcByName = acceptance.InitDataSourceCheck(byName)

		byImageID   = "data.sbercloud_as_configurations.image_id_filter"
...
data "sbercloud_as_configurations" "test" {
}

# Filter by name
locals {
  name = data.sbercloud_as_configurations.test.configurations[0].scaling_configuration_name
}

data "sbercloud_as_configurations" "name_filter" {
  name = local.name
}

locals {
  name_filter_result = [
    for v in data.sbercloud_as_configurations.name_filter.configurations[*].scaling_configuration_name : v == local.name
  ]
}
...
# Filter by image_id
locals {
  image_id = data.sbercloud_as_configurations.test.configurations[0].instance_config[0].image
}

data "sbercloud_as_configurations" "image_id_filter" {
  image_id = local.image_id
}

locals {
  image_id_filter_result = [
    for v in data.sbercloud_as_configurations.image_id_filter.configurations[*].instance_config[0].image : v == local.image_id
  ]
}

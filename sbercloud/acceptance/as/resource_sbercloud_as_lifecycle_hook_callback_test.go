...
func testAccLifecycleHookCallBack_withActionKey(action string) string {
	return fmt.Sprintf(`
resource "sbercloud_as_lifecycle_hook_callback" "test" {
  scaling_group_id        = "%[1]s"
  lifecycle_action_result = "%[2]s"
  lifecycle_action_key    = "%[3]s"
}
`, acceptance.SBC_AS_SCALING_GROUP_ID, action, acceptance.SBC_AS_LIFECYCLE_ACTION_KEY)
}
...
`, acceptance.SBC_AS_SCALING_GROUP_ID, action, acceptance.SBC_AS_INSTANCE_ID, acceptance.SBC_AS_LIFECYCLE_HOOK_NAME)
}

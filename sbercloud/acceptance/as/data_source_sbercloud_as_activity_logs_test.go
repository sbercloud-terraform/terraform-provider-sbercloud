...
func TestAccActivityLogsDataSource_basic(t *testing.T) {
	var (
		dataSourceName = "data.sbercloud_as_activity_logs.test"
		dc             = acceptance.InitDataSourceCheck(dataSourceName)

		byStatus   = "data.sbercloud_as_activity_logs.status_filter"
		dcByStatus = acceptance.InitDataSourceCheck(byStatus)

		byStartTime   = "data.sbercloud_as_activity_logs.start_time_filter"
		dcByStartTime = acceptance.InitDataSourceCheck(byStartTime)

		byEndTime   = "data.sbercloud_as_activity_logs.end_time_filter"
		dcByEndTime = acceptance.InitDataSourceCheck(byEndTime)

		byStartEndTime   = "data.sbercloud_as_activity_logs.start_end_time_filter"
...
func testAccActivityLogsDataSource_basic() string {
	return fmt.Sprintf(`
data "sbercloud_as_activity_logs" "test" {
  scaling_group_id = "%[1]s"
}

# Filter by status
locals {
  status = data.sbercloud_as_activity_logs.test.activity_logs[0].status
}

data "sbercloud_as_activity_logs" "status_filter" {
  scaling_group_id = "%[1]s"
  status           = local.status
}
...
locals {
  status_filter_result = [
    for v in data.sbercloud_as_activity_logs.status_filter.activity_logs[*].status : v == local.status
  ]
}
...
# Filter by time
locals {
  start_time = data.sbercloud_as_activity_logs.test.activity_logs[0].start_time
  end_time   = data.sbercloud_as_activity_logs.test.activity_logs[0].end_time
...
data "sbercloud_as_activity_logs" "start_time_filter" {
  scaling_group_id = "%[1]s"
...
locals {
  start_time_filter_result = [
    for v in data.sbercloud_as_activity_logs.start_time_filter.activity_logs[*].start_time :
...
# Filter by end_time
data "sbercloud_as_activity_logs" "end_time_filter" {
  scaling_group_id = "%[1]s"
...
locals {
  end_time_filter_result = [
    for v in data.sbercloud_as_activity_logs.end_time_filter.activity_logs[*].end_time :
...
data "sbercloud_as_activity_logs" "start_end_time_filter" {
  scaling_group_id = "%[1]s"
  start_time       = local.start_time_utc_before24h
  end_time         = local.end_time_utc_after24h
}
...
locals {
  start_time_filter_result1 = [
    for v in data.sbercloud_as_activity_logs.start_end_time_filter.activity_logs[*].start_time :
    timecmp(local.start_time_utc_before24h, format("%%sZ", replace(v, " ", "T"))) == -1
  ]

  end_time_filter_result1 = [
    for v in data.sbercloud_as_activity_logs.start_end_time_filter.activity_logs[*].end_time :
    timecmp(local.end_time_utc_after24h, format("%%sZ", replace(v, " ", "T"))) == 1
  ]
}

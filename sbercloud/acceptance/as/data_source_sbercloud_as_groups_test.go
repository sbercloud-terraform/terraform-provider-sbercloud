...
func TestAccDataSourceASGroup_basic(t *testing.T) {
	var (
		dataSourceName = "data.sbercloud_as_groups.test"
		dc             = acceptance.InitDataSourceCheck(dataSourceName)

		byName   = "data.sbercloud_as_groups.name_filter"
		dcByName = acceptance.InitDataSourceCheck(byName)

		byScalingConfigurationID   = "data.sbercloud_as_groups.scaling_configuration_id_filter"
		dcByScalingConfigurationID = acceptance.InitDataSourceCheck(byScalingConfigurationID)

		byStatus   = "data.sbercloud_as_groups.status_filter"
		dcByStatus = acceptance.InitDataSourceCheck(byStatus)

		byEnterpriseProjectID   = "data.sbercloud_as_groups.enterprise_project_id_filter"
...
data "sbercloud_as_groups" "test" {
}

# Filter by name
locals {
  name = data.sbercloud_as_groups.test.groups[0].scaling_group_name
}

data "sbercloud_as_groups" "name_filter" {
  name = local.name
}

locals {
  name_filter_result = [
    for v in data.sbercloud_as_groups.name_filter.groups[*].scaling_group_name : v == local.name
  ]
}
...
locals {
  scaling_configuration_id = data.sbercloud_as_groups.test.groups[0].scaling_configuration_id
}

data "sbercloud_as_groups" "scaling_configuration_id_filter" {
  scaling_configuration_id = local.scaling_configuration_id
}

locals {
  scaling_configuration_id_filter_result = [
    for v in data.sbercloud_as_groups.scaling_configuration_id_filter.groups[*].scaling_configuration_id : v == local.scaling_configuration_id
  ]
}
...
# Filter by status
locals {
  status = data.sbercloud_as_groups.test.groups[0].status
}

data "sbercloud_as_groups" "status_filter" {
  status = local.status
}

locals {
  status_filter_result = [
    for v in data.sbercloud_as_groups.status_filter.groups[*].status : v == local.status
  ]
}
...
locals {
  enterprise_project_id = data.sbercloud_as_groups.test.groups[0].enterprise_project_id
}

data "sbercloud_as_groups" "enterprise_project_id_filter" {
  enterprise_project_id = local.enterprise_project_id
}

locals {
  enterprise_project_id_filter_result = [
    for v in data.sbercloud_as_groups.enterprise_project_id_filter.groups[*].enterprise_project_id : v == local.enterprise_project_id
  ]
}

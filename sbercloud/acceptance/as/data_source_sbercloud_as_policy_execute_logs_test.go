...
func TestAccPolicyExecuteLogsDataSource_basic(t *testing.T) {
	var (
		dataSourceName = "data.sbercloud_as_policy_execute_logs.test"
		dc             = acceptance.InitDataSourceCheck(dataSourceName)

		byLogID   = "data.sbercloud_as_policy_execute_logs.log_id_filter"
		dcByLogID = acceptance.InitDataSourceCheck(byLogID)

		byResourceID   = "data.sbercloud_as_policy_execute_logs.resource_id_filter"
		dcByResourceID = acceptance.InitDataSourceCheck(byResourceID)

		byResourceType   = "data.sbercloud_as_policy_execute_logs.resource_type_filter"
		dcByResourceType = acceptance.InitDataSourceCheck(byResourceType)

		byExecuteType   = "data.sbercloud_as_policy_execute_logs.execute_type_filter"
		dcByExecuteType = acceptance.InitDataSourceCheck(byExecuteType)

		byStatus   = "data.sbercloud_as_policy_execute_logs.status_filter"
		dcByStatus = acceptance.InitDataSourceCheck(byStatus)

		byStartTime   = "data.sbercloud_as_policy_execute_logs.start_time_filter"
		dcByStartTime = acceptance.InitDataSourceCheck(byStartTime)

		byEndTime   = "data.sbercloud_as_policy_execute_logs.end_time_filter"
		dcByEndTime = acceptance.InitDataSourceCheck(byEndTime)

		byStartEndTime   = "data.sbercloud_as_policy_execute_logs.start_end_time_filter"
...
func testAccPolicyExecuteLogsDataSource_basic() string {
	return fmt.Sprintf(`
data "sbercloud_as_policy_execute_logs" "test" {
  scaling_policy_id = "%[1]s"
}

// Filter using log_id.
locals {
  log_id = data.sbercloud_as_policy_execute_logs.test.execute_logs[0].id
}

data "sbercloud_as_policy_execute_logs" "log_id_filter" {
  scaling_policy_id = "%[1]s"
  log_id            = local.log_id
}
...
locals {
  log_id_filter_result = [
    for v in data.sbercloud_as_policy_execute_logs.log_id_filter.execute_logs[*].id : v == local.log_id
  ]
}
...
locals {
  scaling_resource_id = data.sbercloud_as_policy_execute_logs.test.execute_logs[0].scaling_resource_id
}

data "sbercloud_as_policy_execute_logs" "resource_id_filter" {
  scaling_policy_id   = "%[1]s"
...
locals {
  scaling_resource_id_filter_result = [
    for v in data.sbercloud_as_policy_execute_logs.resource_id_filter.execute_logs[*].scaling_resource_id :
...
locals {
  scaling_resource_type = data.sbercloud_as_policy_execute_logs.test.execute_logs[0].scaling_resource_type
}

data "sbercloud_as_policy_execute_logs" "resource_type_filter" {
  scaling_policy_id     = "%[1]s"
...
locals {
  scaling_resource_type_filter_result = [
    for v in data.sbercloud_as_policy_execute_logs.resource_type_filter.execute_logs[*].scaling_resource_type :
...
locals {
  execute_type = data.sbercloud_as_policy_execute_logs.test.execute_logs[0].execute_type
}

data "sbercloud_as_policy_execute_logs" "execute_type_filter" {
  scaling_policy_id = "%[1]s"
...
locals {
  execute_type_filter_result = [
    for v in data.sbercloud_as_policy_execute_logs.execute_type_filter.execute_logs[*].execute_type : v == local.execute_type
  ]
}
...
// Filter using status.
locals {
  status = data.sbercloud_as_policy_execute_logs.test.execute_logs[0].status
}

data "sbercloud_as_policy_execute_logs" "status_filter" {
  scaling_policy_id = "%[1]s"
  status            = local.status
}
...
locals {
  status_filter_result = [
    for v in data.sbercloud_as_policy_execute_logs.status_filter.execute_logs[*].status : v == local.status
  ]
}
...
locals {
  execute_time         = data.sbercloud_as_policy_execute_logs.test.execute_logs[0].execute_time
  start_time_before24h = timeadd(local.execute_time, "-24h")
  end_time_after24h    = timeadd(local.execute_time, "24h")
}

data "sbercloud_as_policy_execute_logs" "start_time_filter" {
  scaling_policy_id = "%[1]s"
...
locals {
  start_time_filter_result = [
    for v in data.sbercloud_as_policy_execute_logs.start_time_filter.execute_logs[*].execute_time :
...
data "sbercloud_as_policy_execute_logs" "end_time_filter" {
  scaling_policy_id = "%[1]s"
...
locals {
  end_time_filter_result = [
    for v in data.sbercloud_as_policy_execute_logs.end_time_filter.execute_logs[*].execute_time :
...
data "sbercloud_as_policy_execute_logs" "start_end_time_filter" {
  scaling_policy_id = "%[1]s"
  start_time        = local.start_time_before24h
  end_time          = local.end_time_after24h
}
...
locals {
  start_end_time_filter_result = [
    for v in data.sbercloud_as_policy_execute_logs.start_end_time_filter.execute_logs[*].execute_time :

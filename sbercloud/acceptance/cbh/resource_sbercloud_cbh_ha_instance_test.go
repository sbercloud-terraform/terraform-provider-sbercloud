...
func TestAccCBHHAInstance_basic(t *testing.T) {
	var (
		...
		rName        = "sbercloud_cbh_ha_instance.test"
		defaultEpsId = "0"
		migrateEpsId = acceptance.SBC_ENTERPRISE_PROJECT_ID_TEST
	)
...
resource "sbercloud_cbh_ha_instance" "test" {
  ...
  security_group_id        = join(",", [sbercloud_networking_secgroup.test.id, sbercloud_networking_secgroup.test2.id])
  master_availability_zone = data.sbercloud_availability_zones.test.names[0]
  slave_availability_zone  = data.sbercloud_availability_zones.test.names[1]
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
					resource.TestCheckResourceAttr(rName, "vpc_id", acceptance.SBC_VPC_ID),
					resource.TestCheckResourceAttr(rName, "subnet_id", acceptance.SBC_SUBNET_ID),
					resource.TestCheckResourceAttr(rName, "security_group_id", acceptance.SBC_SECURITY_GROUP_ID),
...
`, name, acceptance.SBC_VPC_ID, acceptance.SBC_SUBNET_ID, acceptance.SBC_SECURITY_GROUP_ID)
}

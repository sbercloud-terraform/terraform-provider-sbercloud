...
func resourceNetworkingSecGroupV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] Create SberCloud Neutron Security Group: %#v", opts)
	...
	log.Printf("[DEBUG] SberCloud Neutron Security Group created: %#v", security_group)
...
func resourceNetworkingSecGroupV2Read(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return CheckDeleted(d, err, "SberCloud Neutron Security group")
	}
...
func resourceNetworkingSecGroupV2Update(d *schema.ResourceData, meta interface{}) error {
	...
	if update {
		...
		if err != nil {
			return fmt.Errorf("Error updating SberCloud SecGroup: %s", err)
		}
	}
...
func resourceNetworkingSecGroupV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error deleting SberCloud Neutron Security Group: %s", err)
	}
...
func waitForSecGroupDelete(networkingClient *golangsdk.ServiceClient, secGroupId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		log.Printf("[DEBUG] Attempting to delete SberCloud Security Group %s.\n", secGroupId)

		r, err := groups.Get(networkingClient, secGroupId).Extract()
		if err != nil {
			if _, ok := err.(golangsdk.ErrDefault404); ok {
				log.Printf("[DEBUG] Successfully deleted SberCloud Neutron Security Group %s", secGroupId)
		...
		log.Printf("[DEBUG] SberCloud Neutron Security Group %s still active.\n", secGroupId)
		return r, "ACTIVE", nil
	}
}

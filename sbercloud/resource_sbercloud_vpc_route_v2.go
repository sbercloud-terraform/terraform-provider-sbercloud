...
func resourceVpcRouteV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating SberCloud vpc route client: %s", err)
	}
	...
	if err != nil {
		return fmt.Errorf("Error creating SberCloud VPC route: %s", err)
	}
	d.SetId(n.RouteID)
...
func resourceVpcRouteV2Read(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		...
		return fmt.Errorf("Error retrieving SberCloud Vpc route: %s", err)
	}
...
func resourceVpcRouteV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error deleting SberCloud Vpc route: %s", err)
	}
...
func waitForVpcRouteDelete(vpcRouteClient *golangsdk.ServiceClient, routeId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		if err != nil {
			if _, ok := err.(golangsdk.ErrDefault404); ok {
				log.Printf("[INFO] Successfully deleted SberCloud vpc route %s", routeId)

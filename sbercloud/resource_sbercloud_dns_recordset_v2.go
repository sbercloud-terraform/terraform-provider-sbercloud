...
func resourceDNSRecordSetV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating SberCloud DNS record set: %s", err)
	}
	...
	log.Printf("[DEBUG] Created SberCloud DNS record set %s: %#v", n.ID, n)
...
func resourceDNSRecordSetV2Read(d *schema.ResourceData, meta interface{}) error {
	...
	if err := d.Set("records", n.Records); err != nil {
		return fmt.Errorf("[DEBUG] Error saving records to state for SberCloud DNS record set (%s): %s", d.Id(), err)
	}
...
func resourceDNSRecordSetV2Update(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error updating SberCloud DNS  record set: %s", err)
	}
...
func resourceDNSRecordSetV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error deleting SberCloud DNS record set: %s", err)
	}
...
func waitForDNSRecordSet(dnsClient *golangsdk.ServiceClient, zoneID, recordsetId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		log.Printf("[DEBUG] SberCloud DNS record set (%s) current status: %s", recordset.ID, recordset.Status)

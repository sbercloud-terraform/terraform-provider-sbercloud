...
func TestAccObsBucketPolicy_basic(t *testing.T) {
	...
	obsName := "sbercloud_obs_bucket.bucket"
	policyName := "sbercloud_obs_bucket_policy.policy"
...
func TestAccObsBucketPolicy_s3(t *testing.T) {
	name := fmt.Sprintf("tf-test-bucket-%d", acctest.RandInt())
	obsName := "sbercloud_obs_bucket.bucket"
	policyName := "sbercloud_obs_bucket_policy.s3_policy"
...
resource "sbercloud_obs_bucket_policy" "policy" {
	bucket = sbercloud_obs_bucket.bucket.bucket
	policy =<<POLICY
{
	"Statement": [{
		"Sid": "test1",

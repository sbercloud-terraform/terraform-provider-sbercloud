		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkingV2SecGroupExists(
						"sbercloud_networking_secgroup_v2.secgroup_1", &security_group),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrPtr(
						"sbercloud_networking_secgroup_v2.secgroup_1", "id", &security_group.ID),
					resource.TestCheckResourceAttr(
						"sbercloud_networking_secgroup_v2.secgroup_1", "name", "terraform_test_security_group_2"),
				),
			},
		},
	})
...
func testAccCheckNetworkingV2SecGroupDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "sbercloud_networking_secgroup_v2" {
			continue
		}
...
resource "sbercloud_networking_secgroup_v2" "secgroup_1" {
  name = "terraform_test_security_group_2"
  description = "terraform security group acceptance test"
}
`
...
resource "sbercloud_networking_secgroup_v2" "secgroup_1" {
	name = "terraform_test_security_group_1"
	description = "terraform security group acceptance test"
